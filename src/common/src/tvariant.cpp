...
void tVariantCreateFromBinary(tVariant *pVar, const char *pz, size_t len, uint32_t type) {
  switch (type) {
    ...
    case TSDB_DATA_TYPE_NCHAR: { // here we get the nchar length from raw binary bits length
      ...
      pVar->wpz = (wchar_t *)calloc(1, (lenInwchar + 1) * TSDB_NCHAR_SIZE);
    ...
    case TSDB_DATA_TYPE_BINARY: {  // todo refactor, extract a method
      pVar->pz = (char*)calloc(len + 1, sizeof(char));
...
void tVariantAssign(tVariant *pDst, const tVariant *pSrc) {
  ...
  if (pSrc->nType == TSDB_DATA_TYPE_BINARY || pSrc->nType == TSDB_DATA_TYPE_NCHAR) {
    ...
    char* p = (char*)realloc(pDst->pz, len);
    assert(p);
  ...
  if (IS_NUMERIC_TYPE(pSrc->nType) || (pSrc->nType == TSDB_DATA_TYPE_BOOL)) {
    ...
    pDst->arr = (SArray*)taosArrayInit(num, sizeof(char*));
    for(size_t i = 0; i < num; i++) {
...
static int32_t toBinary(tVariant *pVariant, char **pDest, int32_t *pDestSize) {
  ...
  if (*pDest == pVariant->pz) {
    pBuf = (char*)calloc(1, INITIAL_ALLOC_SIZE);
  }
  ...
  if (pVariant->nType == TSDB_DATA_TYPE_NCHAR) {
    ...
    if (pBuf != NULL) {
      if (newSize >= INITIAL_ALLOC_SIZE) {
        pBuf = (char*)realloc(pBuf, newSize + 1);
      }
...
static int32_t toNchar(tVariant *pVariant, char **pDest, int32_t *pDestSize) {
  ...
  if (*pDest == pVariant->pz) {
    wchar_t *pWStr = (wchar_t*)calloc(1, (nLen + 1) * TSDB_NCHAR_SIZE);
    ...
    *pDestSize = twcslen(pVariant->wpz);
    ...
    char* tmp = (char*)realloc(pVariant->wpz, (*pDestSize + 1)*TSDB_NCHAR_SIZE);
    assert(tmp != NULL);
    ...
    case TSDB_DATA_TYPE_BINARY: {
      if (!includeLengthPrefix) {
        ...
        if (pVariant->nType == TSDB_DATA_TYPE_NULL) {
          ...
          char *p = (char*)varDataVal(payload);
