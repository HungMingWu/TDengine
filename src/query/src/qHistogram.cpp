...
SHistogramInfo* tHistogramCreate(int32_t numOfEntries) {
  ...
  SHistogramInfo* pHisto = (SHistogramInfo*)malloc(sizeof(SHistogramInfo) + sizeof(SHistBin) * (numOfEntries + 1));
...
#if defined(USE_ARRAYLIST)
  double* pVal = (double*)malloc(num * sizeof(double));
...
SHistogramInfo* tHistogramMerge(SHistogramInfo* pHisto1, SHistogramInfo* pHisto2, int32_t numOfEntries) {
  ...
  SHistBin* pHistoBins = (SHistBin*)calloc(1, sizeof(SHistBin) * (pHisto1->numOfEntries + pHisto2->numOfEntries));
  int32_t i = 0, j = 0, k = 0;
