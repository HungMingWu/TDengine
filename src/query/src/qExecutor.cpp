...
static SResultRow *doPrepareResultRowFromKey(SQueryRuntimeEnv *pRuntimeEnv, SResultRowInfo *pResultRowInfo, char *pData,
  ...
  if (!existed) {
    ...
    if (pResultRowInfo->size >= pResultRowInfo->capacity) {
      ...
      char *t = (char*)realloc(pResultRowInfo->pResult, (size_t)(newCapacity * POINTER_BYTES));
      if (t == NULL) {
  ...
  if (functionId == TSDB_FUNC_ARITHM) {
    ...
    for (int32_t i = 0; i < pQuery->numOfCols; ++i) {
      ...
      for (int32_t k = 0; k < numOfCols; ++k) {  //todo refactor
        SColumnInfoData *p = (SColumnInfoData*)taosArrayGet(pDataBlock, k);
        if (pColMsg->colId == p->info.colId) {
          dataBlock = (char*)p->pData;
          break;
        }
      }
    ...
    if (TSDB_COL_IS_NORMAL_COL(pCol->flag)) {
      ...
      SColumnInfoData *p = (SColumnInfoData*)taosArrayGet(pDataBlock, pColIndex->colIndex);
...
static void saveDataBlockLastRow(SQueryRuntimeEnv* pRuntimeEnv, SDataBlockInfo* pDataBlockInfo, SArray* pDataBlock,
  ...
  for (int32_t k = 0; k < pQuery->numOfCols; ++k) {
    SColumnInfoData *pColInfo = (SColumnInfoData*)taosArrayGet(pDataBlock, k);
...
static void doWindowBorderInterpolation(SQueryRuntimeEnv* pRuntimeEnv, SDataBlockInfo* pDataBlockInfo, SArray *pDataBlock,
  ...
  SColumnInfoData *pColInfo = (SColumnInfoData*)taosArrayGet(pDataBlock, 0);
...
static int32_t setGroupResultOutputBuf(SQueryRuntimeEnv *pRuntimeEnv, char *pData, int16_t type, int16_t bytes, int32_t groupIndex) {
  ...
  if (type == TSDB_DATA_TYPE_BINARY || type == TSDB_DATA_TYPE_NCHAR) {
    if (pResultRow->key == NULL) {
      pResultRow->key = (char*)malloc(varDataTLen(pData));
...
void doRowwiseTimeWindowInterpolation(SQueryRuntimeEnv* pRuntimeEnv, SArray* pDataBlock, TSKEY prevTs, int32_t prevRowIndex, TSKEY curTs, int32_t curRowIndex, TSKEY windowKey,  int32_t type) {
  ...
  for (int32_t k = 0; k < pQuery->numOfOutput; ++k) {
    ...
    SColumnInfoData* pColInfo = (SColumnInfoData*)taosArrayGet(pDataBlock, index);
...
static int32_t setCtxTagColumnInfo(SQueryRuntimeEnv *pRuntimeEnv, SQLFunctionCtx *pCtx) {
  ...
  if (isSelectivityWithTagsQuery(pQuery)) {
    ...
    SQLFunctionCtx **pTagCtx = (SQLFunctionCtx **)calloc(pQuery->numOfOutput, POINTER_BYTES);
    if (pTagCtx == NULL) {
...
static int32_t setupQueryRuntimeEnv(SQueryRuntimeEnv *pRuntimeEnv, int16_t order) {
  ...
  pRuntimeEnv->offset = (uint16_t*)calloc(pQuery->numOfOutput, sizeof(int16_t));
  pRuntimeEnv->rowCellInfoOffset = (int32_t*)calloc(pQuery->numOfOutput, sizeof(int32_t));
  pRuntimeEnv->sasArray = (SArithmeticSupport*)calloc(pQuery->numOfOutput, sizeof(SArithmeticSupport));
  ...
  for (int32_t i = 0; i < pQuery->numOfOutput; ++i) {
    ...
    if (functionId == TSDB_FUNC_ARITHM) {
      pRuntimeEnv->sasArray[i].data = (char**)calloc(pQuery->numOfCols, POINTER_BYTES);
...
static void teardownQueryRuntimeEnv(SQueryRuntimeEnv *pRuntimeEnv) {
  ...
  pRuntimeEnv->pool = (SResultRowPool*)destroyResultRowPool(pRuntimeEnv->pool);
}
...
static void doExchangeTimeWindow(SQInfo* pQInfo, STimeWindow* win) {
  ...
  for(int32_t i = 0; i < t; ++i) {
    SArray* p1 = (SArray*)taosArrayGetP(pQInfo->tableGroupInfo.pGroupList, i);

    size_t len = taosArrayGetSize(p1);
    for(int32_t j = 0; j < len; ++j) {
      STableKeyInfo* pInfo = (STableKeyInfo*)taosArrayGet(p1, j);
...
int32_t loadDataBlockOnDemand(SQueryRuntimeEnv *pRuntimeEnv, SResultRowInfo * pWindowResInfo, void* pQueryHandle, SDataBlockInfo* pBlockInfo, SDataStatis **pStatis, SArray** pDataBlock, uint32_t* status) {
  *status = BLK_DATA_NO_NEEDED;
  ...
  if ((*status) == BLK_DATA_NO_NEEDED) {
    ...
    tsdbRetrieveDataBlockStatisInfo((void **)pQueryHandle, pStatis);
    pCost->loadBlockStatis += 1;

    if (*pStatis == NULL) { // data block statistics does not exist, load data block
      *pDataBlock = tsdbRetrieveDataBlock((void **)pQueryHandle, NULL);
...
static void ensureOutputBufferSimple(SQueryRuntimeEnv* pRuntimeEnv, int32_t capacity) {
  ...
  for (int32_t i = 0; i < pQuery->numOfOutput; ++i) {
    ...
    char *tmp = (char*)realloc(pQuery->sdata[i], bytes * capacity + sizeof(tFilePage));
    if (tmp == NULL) {
      longjmp(pRuntimeEnv->env, TSDB_CODE_QRY_OUT_OF_MEMORY);
    } else {
    ...
    pRuntimeEnv->pCtx[i].aOutputBuf = (char*)pQuery->sdata[i]->data;
  }
  ...
  if (!QUERY_IS_INTERVAL_QUERY(pQuery) && !pRuntimeEnv->groupbyColumn && !isFixedOutputQuery(pRuntimeEnv) && !isTSCompQuery(pQuery)) {
    ...
    if (pQuery->rec.capacity - pQuery->rec.rows < pBlockInfo->rows) {
      ...
      for (int32_t i = 0; i < pQuery->numOfOutput; ++i) {
        ...
        char *tmp = (char*)realloc(pQuery->sdata[i], bytes * newSize + sizeof(tFilePage));
        if (tmp == NULL) {
          longjmp(pRuntimeEnv->env, TSDB_CODE_QRY_OUT_OF_MEMORY);
        } else {
...
static int64_t doScanAllDataBlocks(SQueryRuntimeEnv *pRuntimeEnv) {
  ...
  while (tsdbNextDataBlock((void**)pQueryHandle)) {
...
static void doSetTagValueInParam(void *tsdb, void* pTable, int32_t tagColId, tVariant *tag, int16_t type, int16_t bytes) {
  ...
  if (tagColId == TSDB_TBNAME_COLUMN_INDEX) {
    ...
    tVariantCreateFromBinary(tag, (char*)varDataVal(val), varDataLen(val), TSDB_DATA_TYPE_BINARY);
  } else {
    char* val = (char*)tsdbGetTableTagVal(pTable, tagColId, type, bytes);
    if (val == NULL) {
    ...
    if (type == TSDB_DATA_TYPE_BINARY || type == TSDB_DATA_TYPE_NCHAR) {
      ...
      tVariantCreateFromBinary(tag, (char*)varDataVal(val), varDataLen(val), type);
    } else {
      if (isNull(val, type)) {
        tag->nType = TSDB_DATA_TYPE_NULL;
        return;
...
void copyResToQueryResultBuf(SQInfo *pQInfo, SQuery *pQuery) {
  ...
  pQuery->rec.rows = doCopyToSData(pQInfo, (SResultRow**)pGroupResInfo->pRows->pData, (int32_t) size, &pGroupResInfo->index, TSDB_ORDER_ASC);
}
...
int32_t mergeIntoGroupResultImpl(SGroupResInfo* pGroupResInfo, SArray *pTableList, SQInfo* pQInfo) {
  ...
  if (pGroupResInfo->pRows == NULL) {
    pGroupResInfo->pRows = (SArray*)taosArrayInit(100, POINTER_BYTES);
  }

  posList = (int32_t*)calloc(size, sizeof(int32_t));
  pTableQueryInfoList = (STableQueryInfo**)malloc(POINTER_BYTES * size);
  auto _1 = defer([&] {
  ...
  for (int32_t i = 0; i < size; ++i) {
    STableQueryInfo *item = (STableQueryInfo*)taosArrayGetP(pTableList, i);
...
static void setupQueryRangeForReverseScan(SQInfo* pQInfo) {
  ...
  for(int32_t i = 0; i < numOfGroups; ++i) {
    ...
    SArray *tableKeyGroup = (SArray *)taosArrayGetP(pQInfo->tableGroupInfo.pGroupList, i);

    size_t t = taosArrayGetSize(group);
    for (int32_t j = 0; j < t; ++j) {
      STableQueryInfo *pCheckInfo = (STableQueryInfo *)taosArrayGetP(group, j);
      updateTableQueryInfoForReverseScan(pQuery, pCheckInfo);

      // update the last key in tableKeyInfo list, the tableKeyInfo is used to build the tsdbQueryHandle and decide
      // the start check timestamp of tsdbQueryHandle
      STableKeyInfo *pTableKeyInfo = (STableKeyInfo *)taosArrayGet(tableKeyGroup, j);
...
static SQueryStatusInfo getQueryStatusInfo(SQueryRuntimeEnv *pRuntimeEnv, TSKEY start) {
  ...
  info.status = pQuery->status;
  info.windowIndex = pRuntimeEnv->windowResInfo.curIndex;
  info.lastKey = start;
...
static void restoreTimeWindow(STableGroupInfo* pTableGroupInfo, STsdbQueryCond* pCond) {
  ...
  SArray* pTableKeyGroup = (SArray*)taosArrayGetP(pTableGroupInfo->pGroupList, 0);
  STableKeyInfo* pKeyInfo = (STableKeyInfo*)taosArrayGet(pTableKeyGroup, 0);
...
static STableQueryInfo *createTableQueryInfo(SQueryRuntimeEnv *pRuntimeEnv, void* pTable, STimeWindow win, void* buf) {
  ...
  STableQueryInfo *pTableQueryInfo = (STableQueryInfo*)buf;
...
static void doCopyQueryResultToMsg(SQInfo *pQInfo, int32_t numOfRows, char *data) {
  ...
  *(int32_t*)data = htonl(numOfTables);
  ...
  int32_t total = 0;
  STableIdInfo *item = (STableIdInfo *)taosHashIterate((SHashObj *)pQInfo->arrTableIdInfo, NULL);
  ...
  while(item) {
    ...
    item = (STableIdInfo *)taosHashIterate((SHashObj *)pQInfo->arrTableIdInfo, item);
  }
...
static void updateOffsetVal(SQueryRuntimeEnv *pRuntimeEnv, SDataBlockInfo *pBlockInfo) {
  ...
  SArray *         pDataBlock = (SArray *)tsdbRetrieveDataBlock((void**)pRuntimeEnv->pQueryHandle, NULL);
  SColumnInfoData *pColInfoData = (SColumnInfoData *)taosArrayGet(pDataBlock, 0);
...
static TSKEY doSkipIntervalProcess(SQueryRuntimeEnv* pRuntimeEnv, STimeWindow* win, SDataBlockInfo* pBlockInfo, STableQueryInfo* pTableQueryInfo) {
  ...
  if ((tw.skey <= pBlockInfo->window.ekey && QUERY_IS_ASC_QUERY(pQuery)) ||
    ...
    SArray *         pDataBlock = tsdbRetrieveDataBlock((void**)pRuntimeEnv->pQueryHandle, NULL);
    SColumnInfoData *pColInfoData = (SColumnInfoData *)taosArrayGet(pDataBlock, 0);

    tw = *win;
    int32_t startPos =
        getNextQualifiedWindow(pRuntimeEnv, &tw, pBlockInfo, (int64_t*)pColInfoData->pData, binarySearchForKey, -1);
    assert(startPos >= 0);
...
static bool skipTimeInterval(SQueryRuntimeEnv *pRuntimeEnv, TSKEY* start) {
  ...
  SDataBlockInfo blockInfo = SDATA_BLOCK_INITIALIZER;
  while (tsdbNextDataBlock((void**)pRuntimeEnv->pQueryHandle)) {
    tsdbRetrieveDataBlockInfo((void**)pRuntimeEnv->pQueryHandle, &blockInfo);
      ...
      if ((tw.skey <= blockInfo.window.ekey && ascQuery) || (tw.ekey >= blockInfo.window.skey && !ascQuery)) {
        ...
        if (pQuery->limit.offset == 0) {
          *start = doSkipIntervalProcess(pRuntimeEnv, &win, &blockInfo, pTableQueryInfo);
          ...
          tw = win;
          int32_t startPos =
              getNextQualifiedWindow(pRuntimeEnv, &tw, &blockInfo, (int64_t*)pColInfoData->pData, binarySearchForKey, -1);
          assert(startPos >= 0);
...
static int32_t setupQueryHandle(STsdbRepo *tsdb, SQInfo *pQInfo, bool isSTableQuery) {
  ...
  if (!isSTableQuery
    ...
    STableQueryInfo* pCheckInfo = (STableQueryInfo*)taosArrayGetP(pa, 0);
...
static bool multiTableMultioutputHelper(SQInfo *pQInfo, int32_t index) {
  ...
  STableQueryInfo* pCheckInfo = (STableQueryInfo* )taosArrayGetP(group, index);
  ...
  STsdbQueryCond cond;
  cond.twindow.skey = pCheckInfo->lastKey;
  cond.twindow.ekey = pCheckInfo->win.ekey;
  cond.order = pQuery->order.order;
  cond.colList = pQuery->colList;
  cond.numOfCols = pQuery->numOfCols;

  // todo refactor
  SArray *g1 = (SArray*)taosArrayInit(1, POINTER_BYTES);
  SArray *tx = (SArray*)taosArrayInit(1, sizeof(STableKeyInfo));
...
static void updateTableIdInfo(SQuery* pQuery, SHashObj* pTableIdInfo) {
  ...
  STableIdInfo* idinfo = (STableIdInfo* )taosHashGet(pTableIdInfo, &tidInfo.tid, sizeof(tidInfo.tid));
  if (idinfo != NULL) {
...
static void sequentialTableProcess(SQInfo *pQInfo) {
  ...
  if (isPointInterpoQuery(pQuery)) {
    ...
    while (pQInfo->groupIndex < numOfGroups) {
      SArray *group = (SArray *)taosArrayGetP(pQInfo->tableGroupInfo.pGroupList, pQInfo->groupIndex);
      ...
      SArray *g1 = (SArray*)taosArrayInit(1, POINTER_BYTES);
      SArray *tx = (SArray*)taosArrayClone(group);
      taosArrayPush(g1, &tx);
      ...
      SArray *s = tsdbGetQueriedTableList((void**)pRuntimeEnv->pQueryHandle);
      ...
      pQuery->current = (STableQueryInfo*)taosArrayGetP(first, 0);
    ...
    int32_t step = GET_FORWARD_DIRECTION_FACTOR(pQuery->order.order);
    SQueryCostInfo *summary = &pRuntimeEnv->summary;
    while ((hasMoreBlock = tsdbNextDataBlock((void**)pQueryHandle)) == true) {
      summary->totalBlocks += 1;
    ...
    while (pQInfo->tableIndex < pQInfo->tableqinfoGroupInfo.numOfTables) {
      ...
      pQuery->current = (STableQueryInfo*)taosArrayGetP(group, pQInfo->tableIndex);
  ...
  if (QUERY_IS_INTERVAL_QUERY(pQuery)) {
    ...
    for (int32_t i = 0; i < numOfGroup; ++i) {
      ...
      for (int32_t j = 0; j < num; ++j) {
        STableQueryInfo* item = (STableQueryInfo*)taosArrayGetP(group, j);
...
static void doSecondaryArithmeticProcess(SQuery* pQuery) {
  ...
  tFilePage **data = (tFilePage **)calloc(pQuery->numOfExpr2, POINTER_BYTES);
...
static void tableQueryImpl(SQInfo *pQInfo) {
  ...
  STableQueryInfo* item = (STableQueryInfo*)taosArrayGetP(g, 0);
  pQuery->current = item;
...
static char *createTableIdList(SQueryTableMsg *pQueryMsg, char *pMsg, SArray **pTableIdList) {
  ...
  *pTableIdList = (SArray *)taosArrayInit(pQueryMsg->numOfTables, sizeof(STableIdInfo));
...
static int32_t convertQueryMsg(SQueryTableMsg *pQueryMsg, SArray **pTableIdList, SSqlFuncMsg ***pExpr,
  ...
  for (int32_t col = 0; col < pQueryMsg->numOfCols; ++col) {
    ...
    if (numOfFilters > 0) {
      pColInfo->filters = (SColumnFilterInfo*)calloc(numOfFilters, sizeof(SColumnFilterInfo));
  ...
  *pExpr = (SSqlFuncMsg**)calloc(pQueryMsg->numOfOutput, POINTER_BYTES);
  if (*pExpr == NULL) {
    code = TSDB_CODE_QRY_OUT_OF_MEMORY;
    return code;
  }
  ...
  if (pQueryMsg->secondStageOutput) {
    pExprMsg = (SSqlFuncMsg *)pMsg;
    *pSecStageExpr = (SSqlFuncMsg**)calloc(pQueryMsg->secondStageOutput, POINTER_BYTES);
  ...
  if (pQueryMsg->numOfGroupCols > 0) {  // group by tag columns
    *groupbyCols = (SColIndex*)malloc(pQueryMsg->numOfGroupCols * sizeof(SColIndex));
    if (*groupbyCols == NULL) {
      code = TSDB_CODE_QRY_OUT_OF_MEMORY;
      return code;
    }
  ...
  if (pQueryMsg->numOfTags > 0) {
    (*tagCols) = (SColumnInfo*)calloc(1, sizeof(SColumnInfo) * pQueryMsg->numOfTags);
    if (*tagCols == NULL) {
      code = TSDB_CODE_QRY_OUT_OF_MEMORY;
      return code;
    }
...
static int32_t createFilterInfo(void *pQInfo, SQuery *pQuery) {
  ...
  pQuery->pFilterInfo = (SSingleColumnFilterInfo*)calloc(1, sizeof(SSingleColumnFilterInfo) * pQuery->numOfFilterCols);
  ...
  for (int32_t i = 0, j = 0; i < pQuery->numOfCols; ++i) {
    if (pQuery->colList[i].numOfFilters > 0) {
      ...
      pFilterInfo->pFilters = (SColumnFilterElem*)calloc(pFilterInfo->numOfFilters, sizeof(SColumnFilterElem));
...
static SQInfo *createQInfoImpl(SQueryTableMsg *pQueryMsg, SSqlGroupbyExpr *pGroupbyExpr, SExprInfo *pExprs,
  ...
  auto  _1 = defer([&] { tsdbDestroyTableGroup(pTableGroupInfo); });
  if (pQInfo == NULL) {
    return nullptr;
  }
  ...
  auto _3 = defer([&] { freeQInfo(pQInfo); });
  ...
  pQuery->colList = (SColumnInfo *)calloc(numOfCols, sizeof(SSingleColumnFilterInfo));
  ...
  if (pQuery->fillType != TSDB_FILL_NONE) {
    pQuery->fillVal = (int64_t *)malloc(sizeof(int64_t) * pQuery->numOfOutput);
    if (pQuery->fillVal == NULL) {
      return nullptr;
    }
  ...
  if (pTableGroupInfo->pGroupList != NULL) {
    ...
    pQInfo->tableqinfoGroupInfo.pGroupList = (SArray*)taosArrayInit(numOfGroups, POINTER_BYTES);
  ...
  pQInfo->runtimeEnv.keyBuf = (char*)malloc(TSDB_MAX_BYTES_PER_ROW);
  pQInfo->runtimeEnv.pool = initResultRowPool(getResultRowSize(&pQInfo->runtimeEnv));
  pQInfo->runtimeEnv.prevRow = (char**)malloc(POINTER_BYTES * pQuery->numOfCols + srcSize);
  ...
  pQInfo->pBuf = (STableQueryInfo*)calloc(pTableGroupInfo->numOfTables, sizeof(STableQueryInfo));
  if (pQInfo->pBuf == NULL) {
    return nullptr;
  }
  ...
  int32_t index = 0;
  for(int32_t i = 0; i < numOfGroups; ++i) {
    SArray* pa = (SArray *)taosArrayGetP(pQInfo->tableGroupInfo.pGroupList, i);

    size_t s = taosArrayGetSize(pa);
    SArray* p1 = (SArray*)taosArrayInit(s, POINTER_BYTES);
    if (p1 == NULL) {
      return nullptr;
    }
    ...
    for(int32_t j = 0; j < s; ++j) {
      STableKeyInfo* info = (STableKeyInfo*)taosArrayGet(pa, j);
...
static void doDestroyTableQueryInfo(STableGroupInfo* pTableqinfoGroupInfo) {
  if (pTableqinfoGroupInfo->pGroupList != NULL) {
    ...
    for (int32_t i = 0; i < numOfGroups; ++i) {
      SArray *p = (SArray *)taosArrayGetP(pTableqinfoGroupInfo->pGroupList, i);

      size_t num = taosArrayGetSize(p);
      for(int32_t j = 0; j < num; ++j) {
        STableQueryInfo* item = (STableQueryInfo*)taosArrayGetP(p, j);
...
static void freeQInfo(SQInfo *pQInfo) {
  ...
  if (pQuery != NULL) {
    ...
    pQuery->pExpr1 = (SExprInfo*)destroyQueryFuncExpr(pQuery->pExpr1, pQuery->numOfOutput);
    pQuery->pExpr2 = (SExprInfo*)destroyQueryFuncExpr(pQuery->pExpr2, pQuery->numOfExpr2);
...
int32_t qCreateQueryInfo(STsdbRepo *tsdb, int32_t vgId, SQueryTableMsg *pQueryMsg, qinfo_t *pQInfo) {
  ...
  if (TSDB_QUERY_HAS_TYPE(pQueryMsg->queryType, TSDB_QUERY_TYPE_TABLE_QUERY)) {
    STableIdInfo *id = (STableIdInfo *)taosArrayGet(pTableIdList, 0);
  ...
  code = initQInfo(pQueryMsg, tsdb, vgId, (SQInfo*)*pQInfo, isSTableQuery);
  return code;
}
...
static void buildTagQueryResult(SQInfo* pQInfo) {
  ...
  if (functionId == TSDB_FUNC_TID_TAG) { // return the tags & table Id
    ...
    while(pQInfo->tableIndex < num && count < pQuery->rec.capacity) {
      ...
      STableQueryInfo *item = (STableQueryInfo*)taosArrayGetP(pa, i);

      char *output = pQuery->sdata[0]->data + count * rsize;
      varDataSetLen(output, rsize - VARSTR_HEADER_SIZE);

      output = (char*)varDataVal(output);
      ...
      *(int32_t *)output = pQInfo->vgId;
      ...
      if (pExprInfo->base.colInfo.colId == TSDB_TBNAME_COLUMN_INDEX) {
        ...
        char* data = (char*)tsdbGetTableTagVal(item->pTable, pExprInfo->base.colInfo.colId, type, bytes);
    ...
    while(pQInfo->tableIndex < num && count < maxNumOfTables) {
      ...
      for(int32_t j = 0; j < pQuery->numOfOutput; ++j) {
        ...
        if (pExprInfo[j].base.colInfo.colId == TSDB_TBNAME_COLUMN_INDEX) {
          ...
          data = (char*)tsdbGetTableTagVal(item->pTable, pExprInfo[j].base.colInfo.colId, type, bytes);
...
void** qRegisterQInfo(void* pMgmt, uint64_t qInfo) {
  ...
  SQueryMgmt *pQueryMgmt = (SQueryMgmt *)pMgmt;
...
void** qAcquireQInfo(void* pMgmt, uint64_t _key) {
  ...
  void** handle = (void**)taosCacheAcquireByKey(pQueryMgmt->qinfoPool, &key, sizeof(TSDB_CACHE_PTR_TYPE));
  if (handle == NULL || *handle == NULL) {
    terrno = TSDB_CODE_QRY_INVALID_QHANDLE;
    return NULL;
  } else {
...
void** qReleaseQInfo(void* pMgmt, void* pQInfo, bool freeHandle) {
  ...
  taosCacheRelease(pQueryMgmt->qinfoPool, (void**)pQInfo, freeHandle);
  return 0;
}
