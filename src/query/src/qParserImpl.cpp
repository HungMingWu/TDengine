...
tSQLExprList *tSqlExprListAppend(tSQLExprList *pList, tSQLExpr *pNode, SStrToken *pToken) {
  if (pList == NULL) {
    pList = (tSQLExprList*)calloc(1, sizeof(tSQLExprList));
  }

  if (pList->nAlloc <= pList->nExpr) {
    pList->nAlloc = (pList->nAlloc << 1u) + 4;
    pList->a = (tSqlExprItem*)realloc(pList->a, pList->nAlloc * sizeof(pList->a[0]));
    if (pList->a == 0) {
  ...
  if (pNode || pToken) {
    ...
    if (pToken) {  // set the as clause
      pItem->aliasName = (char*)malloc(pToken->n + 1);
...
tSQLExpr *tSqlExprIdValueCreate(SStrToken *pToken, int32_t optrType) {
  tSQLExpr *pSqlExpr = (tSQLExpr*)calloc(1, sizeof(tSQLExpr));
...
tSQLExpr *tSqlExprCreateFunction(tSQLExprList *pList, SStrToken *pFuncToken, SStrToken *endToken, int32_t optType) {
  ...
  tSQLExpr *pExpr = (tSQLExpr*)calloc(1, sizeof(tSQLExpr));
...
tSQLExpr *tSqlExprCreate(tSQLExpr *pLeft, tSQLExpr *pRight, int32_t optrType) {
  ...
  if ((pLeft != NULL && pRight != NULL) &&
    ...
    tSQLExpr *pRSub = (tSQLExpr*)calloc(1, sizeof(tSQLExpr));
    ...
    if (pRight == NULL) {
      pRight = (tSQLExpr*)calloc(1, sizeof(tSQLExpr));
    }
...
SArray *tVariantListAppendToken(SArray *pList, SStrToken *pToken, uint8_t order) {
  if (pList == NULL) {
    pList = (SArray*)taosArrayInit(4, sizeof(tVariantListItem));
  }
...
SQuerySQL *tSetQuerySqlElems(SStrToken *pSelectToken, tSQLExprList *pSelection, SArray *pFrom, tSQLExpr *pWhere,
  ...
  SQuerySQL *pQuery = (SQuerySQL*)calloc(1, sizeof(SQuerySQL));
...
SCreateTableSQL *tSetCreateSqlElems(SArray *pCols, SArray *pTags, SQuerySQL *pSelect, int32_t type) {
  SCreateTableSQL *pCreate = (SCreateTableSQL*)calloc(1, sizeof(SCreateTableSQL));
...
SAlterTableSQL *tAlterTableSqlElems(SStrToken *pTableName, SArray *pCols, SArray *pVals, int32_t type) {
  SAlterTableSQL *pAlterTable = (SAlterTableSQL*)calloc(1, sizeof(SAlterTableSQL));
...
void SqlInfoDestroy(SSqlInfo *pInfo) {
  ...
  if (pInfo->type == TSDB_SQL_SELECT) {
    ...
    pInfo->pCreateTableInfo = (SCreateTableSQL*)destroyCreateTableSql(pInfo->pCreateTableInfo);
...
SSubclauseInfo* setSubclause(SSubclauseInfo* pSubclause, void *pSqlExprInfo) {
  if (pSubclause == NULL) {
    pSubclause = (SSubclauseInfo*)calloc(1, sizeof(SSubclauseInfo));
  }
  
  int32_t newSize = pSubclause->numOfClause + 1;
  char* tmp = (char*)realloc(pSubclause->pClause, newSize * POINTER_BYTES);
  if (tmp == NULL) {
    return pSubclause;
  }
  ...
  pSubclause->pClause[newSize - 1] = (SQuerySQL*)pSqlExprInfo;
...
SSqlInfo*setSqlInfo(SSqlInfo *pInfo, void *pSqlExprInfo, SStrToken *pTableName, int32_t type) {
  ...
  if (type == TSDB_SQL_SELECT) {
    ...
    pInfo->pCreateTableInfo = (SCreateTableSQL*)pSqlExprInfo;
  }
...
SSubclauseInfo* appendSelectClause(SSubclauseInfo *pQueryInfo, void *pSubclause) {
  char* tmp = (char*)realloc(pQueryInfo->pClause, (pQueryInfo->numOfClause + 1) * POINTER_BYTES);
  if (tmp == NULL) {  // out of memory
    return pQueryInfo;
  }
  
  pQueryInfo->pClause = (SQuerySQL**) tmp;
  pQueryInfo->pClause[pQueryInfo->numOfClause++] = (SQuerySQL*)pSubclause;
...
void tTokenListBuyMoreSpace(tDCLSQL *pTokenList) {
  if (pTokenList->nAlloc <= pTokenList->nTokens) {  //
    ...
    pTokenList->a = (SStrToken*)realloc(pTokenList->a, pTokenList->nAlloc * sizeof(pTokenList->a[0]));
    if (pTokenList->a == 0) {
...
tDCLSQL *tTokenListAppend(tDCLSQL *pTokenList, SStrToken *pToken) {
  ...
  if (pTokenList == NULL) pTokenList = (tDCLSQL*)calloc(1, sizeof(tDCLSQL));
...
void setShowOptions(SSqlInfo *pInfo, int32_t type, SStrToken* prefix, SStrToken* pPatterns) {
  if (pInfo->pDCLInfo == NULL) {
    pInfo->pDCLInfo = (tDCLSQL*)calloc(1, sizeof(tDCLSQL));
  }
