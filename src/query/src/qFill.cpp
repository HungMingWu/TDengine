...
static void initBeforeAfterDataBuf(SFillInfo* pFillInfo, char** next) {
  ...
  *next = (char*)calloc(1, pFillInfo->rowSize);
  ...
  while (pFillInfo->numOfCurrent < outputRows) {
    ...
    if (((pFillInfo->currentKey < ts && FILL_IS_ASC_FILL(pFillInfo)) || (pFillInfo->currentKey > ts && !FILL_IS_ASC_FILL(pFillInfo))) &&
      ...
      for (int32_t i = 0; i < pFillInfo->numOfCols; ++i) {
        ...
        char* output = (char*)elePtrAt(data[i]->data, pCol->col.bytes, pFillInfo->numOfCurrent);
        char* src = (char*)elePtrAt(srcData[i], pCol->col.bytes, pFillInfo->index);
...
SFillInfo* taosInitFillInfo(int32_t order, TSKEY skey, int32_t numOfTags, int32_t capacity, int32_t numOfCols,
  ...
  SFillInfo* pFillInfo = (SFillInfo*)calloc(1, sizeof(SFillInfo));
  ...
  pFillInfo->pData = (char**)malloc(POINTER_BYTES * numOfCols);
  if (numOfTags > 0) {
    pFillInfo->pTags = (SFillTagColInfo*)calloc(pFillInfo->numOfTags, sizeof(SFillTagColInfo));
...
void taosFillSetStartInfo(SFillInfo* pFillInfo, int32_t numOfRows, TSKEY endKey) {
  ...
  if (pFillInfo->alloc < numOfRows) {
    for(int32_t i = 0; i < pFillInfo->numOfCols; ++i) {
      char* tmp = (char*)realloc(pFillInfo->pData[i], numOfRows*pFillInfo->pFillCol[i].col.bytes);
