...
void arithmeticTreeTraverse(tExprNode *pExprs, int32_t numOfRows, char *pOutput, void *param, int32_t order,
  ...
  char *pRightOutput = (char*)malloc(sizeof(int64_t) * numOfRows);
  char *pdata = (char*)malloc(sizeof(int64_t) * numOfRows);
...
static tExprNode* exprTreeFromBinaryImpl(SBufferReader* br) {
  ...
  tExprNode* pExpr = (tExprNode *)exception_calloc(1, sizeof(tExprNode));
  ...
  if (pExpr->nodeType == TSQL_NODE_VALUE) {
    tVariant* pVal = (tVariant*)exception_calloc(1, sizeof(tVariant));
    pExpr->pVal = pVal;
  
    pVal->nType = tbufReadUint32(br);
    if (pVal->nType == TSDB_DATA_TYPE_BINARY) {
      tbufReadToBuffer(br, &pVal->nLen, sizeof(pVal->nLen));
      pVal->pz = (char*)calloc(1, pVal->nLen + 1);
    ...
    SSchema* pSchema = (SSchema*)exception_calloc(1, sizeof(SSchema));
...
tExprNode* exprTreeFromBinary(const void* data, size_t size) {
  ...
  SBufferReader br = tbufInitReader((const char*)data, size, false);
...
tExprNode* exprTreeFromTableName(const char* tbnameCond) {
  ...
  tExprNode* expr = (tExprNode*)exception_calloc(1, sizeof(tExprNode));
  CLEANUP_PUSH_VOID_PTR_PTR(true, tExprNodeDestroy, expr, NULL);

  expr->nodeType = TSQL_NODE_EXPR;

  tExprNode* left = (tExprNode*)exception_calloc(1, sizeof(tExprNode));
  ...
  *pSchema = tscGetTbnameColumnSchema();
  ...
  tExprNode* right = (tExprNode*)exception_calloc(1, sizeof(tExprNode));
  ...
  if (strncmp(tbnameCond, QUERY_COND_REL_PREFIX_LIKE, QUERY_COND_REL_PREFIX_LIKE_LEN) == 0) {
    ...
    pVal->pz = (char*)exception_malloc(len);
    ...
    pVal->arr = (SArray*)taosArrayInit(2, POINTER_BYTES);
    ...
    for (const char *e = cond; *e != 0; e++) {
      if (*e == TS_PATH_DELIMITER[0]) {
        ...
        size_t len = e - cond;
        char* p = (char*)exception_malloc(len + VARSTR_HEADER_SIZE);
    ...
    if (*cond != 0) {
      ...
      char* p = (char*)exception_malloc(len);
