...
STSBuf* tsBufCreate(bool autoDelete, int32_t order) {
  STSBuf* pTSBuf = (STSBuf*)calloc(1, sizeof(STSBuf));
  if (pTSBuf == NULL) {
    return NULL;
  }
...
STSBuf* tsBufCreateFromFile(const char* path, bool autoDelete) {
  ...
  if (header.numOfGroup > pTSBuf->numOfAlloc) {
    ...
    STSGroupBlockInfoEx* tmp = (STSGroupBlockInfoEx*)realloc(pTSBuf->pData, sizeof(STSGroupBlockInfoEx) * pTSBuf->numOfAlloc);
    if (tmp == NULL) {
      tsBufDestroy(pTSBuf);
      return NULL;
    }
...
static void shrinkBuffer(STSList* ptsData) {
  ...
  if (ptsData->allocSize >= ptsData->threshold * 2) {
    ptsData->rawBuf = (char*)realloc(ptsData->rawBuf, MEM_BUF_SIZE);
...
static void expandBuffer(STSList* ptsData, int32_t inputSize) {
  if (ptsData->allocSize - ptsData->len < inputSize) {
    ...
    char*   tmp = (char*)realloc(ptsData->rawBuf, (size_t)newSize);
    if (tmp == NULL) {
      // todo
    }
...
STSBlock* readDataFromDisk(STSBuf* pTSBuf, int32_t order, bool decomp) {
  ...
  size_t sz = 0;
  if (pBlock->tag.nType == TSDB_DATA_TYPE_BINARY || pBlock->tag.nType == TSDB_DATA_TYPE_NCHAR) {
    char* tp = (char*)realloc(pBlock->tag.pz, pBlock->tag.nLen + 1);
    assert(tp != NULL);
...
void tsBufResetPos(STSBuf* pTSBuf) {
  ...
  pTSBuf->cur = (STSCursor){.vgroupIndex = -1, .blockIndex = -1, .tsIndex = -1, .order = pTSBuf->cur.order};
}
...
int32_t tsBufMerge(STSBuf* pDestBuf, const STSBuf* pSrcBuf) {
  ...
  if (id != tsBufGetLastGroupInfo(pDestBuf)->info.id) {
    ...
    if (pDestBuf->numOfAlloc < newSize) {
      ...
      STSGroupBlockInfoEx* tmp = (STSGroupBlockInfoEx*)realloc(pDestBuf->pData, sizeof(STSGroupBlockInfoEx) * newSize);
      if (tmp == NULL) {
        return -1;
      }
...
static STSBuf* allocResForTSBuf(STSBuf* pTSBuf) {
  ...
  pTSBuf->pData = (STSGroupBlockInfoEx*)calloc(pTSBuf->numOfAlloc, sizeof(STSGroupBlockInfoEx));
  if (pTSBuf->pData == NULL) {
    tsBufDestroy(pTSBuf);
    return NULL;
  }
  
  pTSBuf->tsData.rawBuf = (char*)malloc(MEM_BUF_SIZE);
  ...
  pTSBuf->assistBuf = (char*)malloc(MEM_BUF_SIZE);
  if (pTSBuf->assistBuf == NULL) {
    tsBufDestroy(pTSBuf);
    return NULL;
  }
  
  pTSBuf->block.payload = (char*)malloc(MEM_BUF_SIZE);
...
void tsBufGetGroupIdList(STSBuf* pTSBuf, int32_t* num, int32_t** id) {
  ...
  *id = NULL;
  ...
  (*id) = (int32_t*)malloc(tsBufGetNumOfGroup(pTSBuf) * sizeof(int32_t));
