...
int32_t initResultRowInfo(SResultRowInfo *pResultRowInfo, int32_t size, int16_t type) {
  ...
  pResultRowInfo->pResult = (SResultRow**)calloc(pResultRowInfo->capacity, POINTER_BYTES);
...
SResultRowPool* initResultRowPool(size_t size) {
  SResultRowPool* p = (SResultRowPool *)calloc(1, sizeof(SResultRowPool));
  if (p == NULL) {
    return NULL;
  }
  ...
  p->pData = (SArray*)taosArrayInit(8, POINTER_BYTES);
  return p;
}
...
SResultRow* getNewResultRow(SResultRowPool* p) {
  ...
  if (p->position.pos == 0) {
    ...
    void** pBlock = (void**)taosArrayGet(p->pData, last - 1);
    ptr = ((char*) (*pBlock)) + p->elemSize * p->position.pos;
  }

  p->position.pos = (p->position.pos + 1)%p->numOfElemPerBlock;
  initResultRow((SResultRow*)ptr);

  return (SResultRow*)ptr;
}
...
void* destroyResultRowPool(SResultRowPool* p) {
  ...
  for(int32_t i = 0; i < size; ++i) {
    void** ptr = (void**)taosArrayGet(p->pData, i);
    tfree(*ptr);
  }
