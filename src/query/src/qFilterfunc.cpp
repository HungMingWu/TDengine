...
bool equalOperator(SColumnFilterElem *pFilter, const char *minval, const char *maxval, int16_t type) {
  ...
  if (IS_SIGNED_NUMERIC_TYPE(type) || type == TSDB_DATA_TYPE_BOOL) {
    ...
    return strncmp((char *)pFilterInfo->pz, (char*)varDataVal(minval), varDataLen(minval)) == 0;
    ...
    return wcsncmp((wchar_t *)pFilterInfo->pz, (wchar_t *)varDataVal(minval), varDataLen(minval) / TSDB_NCHAR_SIZE) ==
           0;
  } else {
    return false;
  }
...
bool likeOperator(SColumnFilterElem *pFilter, const char *minval, const char *maxval, int16_t type) {
  if (type == TSDB_DATA_TYPE_BINARY) {
    ...
    return patternMatch((char *)pFilter->filterInfo.pz, (char*)varDataVal(minval), varDataLen(minval), &info) == TSDB_PATTERN_MATCH;
  } else if (type == TSDB_DATA_TYPE_NCHAR) {
    SPatternCompareInfo info = PATTERN_COMPARE_INFO_INITIALIZER;
    return WCSPatternMatch((wchar_t*)pFilter->filterInfo.pz, (wchar_t*)varDataVal(minval), varDataLen(minval)/TSDB_NCHAR_SIZE, &info) == TSDB_PATTERN_MATCH;
  } else {
    return false;
  }
}
...
bool notEqualOperator(SColumnFilterElem *pFilter, const char *minval, const char *maxval, int16_t type) {
  ...
  if (IS_SIGNED_NUMERIC_TYPE(type) || type == TSDB_DATA_TYPE_BOOL) {
    ...
    return wcsncmp((wchar_t *)pFilterInfo->pz, (wchar_t*)varDataVal(minval), varDataLen(minval)/TSDB_NCHAR_SIZE) != 0;
  } else {
    return false;
  }
}
