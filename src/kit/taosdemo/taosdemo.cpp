...
typedef struct DemoArguments {
  ...
  static error_t parse_opt(int key, char *arg, struct argp_state *state) {
    ...
    SDemoArguments *arguments = (SDemoArguments*)state->input;
    wordexp_t full_path;
    char **sptr;
    switch (key) {
      case 'h':
...
int main(int argc, char *argv[]) {
  ...
  enum MODE query_mode = (MODE)arguments.mode;
  ...
  info *infos = (info*)malloc(threads * sizeof(info));
  ...
  if (false == insert_only) {
    // query data
    pthread_t read_id;
    info *rInfo = (info*)malloc(sizeof(info));
...
int32_t generateData(char *res, char **data_type, int num_of_cols, int64_t timestamp, int len_of_binary) {
  ...
  for (int i = 0; i < num_of_cols; i++) {
    if (strcasecmp(data_type[i % c], "tinyint") == 0) {
      ...
      char *s = (char*)malloc(len_of_binary);
