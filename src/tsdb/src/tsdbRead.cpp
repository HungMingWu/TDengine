...
struct STableCheckInfo {
 ...
 public:
  bool operator<(const STableCheckInfo& rhs) {
    return tableId.tid < rhs.tableId.tid;
  }
};
...
typedef struct STsdbQueryHandle {
  ...
  std::vector<STableCheckInfo> pTableCheckInfo;
  int32_t        activeIndex;
...
SDataBlockInfo GET_FILE_DATA_BLOCK_INFO(STableCheckInfo* _checkInfo, SCompBlock* _block) {
  SDataBlockInfo result;
  result.window.skey = _block->keyFirst;
  result.window.ekey = _block->keyLast;
  result.numOfCols = _block->numOfCols;
  result.rows = _block->numOfRows;
  result.tid = _checkInfo->tableId.tid;
  result.uid = _checkInfo->tableId.uid;
  return result;
}
...
static SArray* getColumnIdList(STsdbQueryHandle* pQueryHandle) {
  ...
  SArray* pIdList = (SArray*)taosArrayInit(numOfCols, sizeof(int16_t));
  for (int32_t i = 0; i < numOfCols; ++i) {
    SColumnInfoData* pCol = (SColumnInfoData*)taosArrayGet(pQueryHandle->pColumns, i);
...
static void tsdbMayUnTakeMemSnapshot(STsdbQueryHandle* pQueryHandle) {
  ...
  if (--pMemRef->ref == 0) {
    tsdbUnTakeMemSnapShot(pQueryHandle->pTsdb, (SMemTable*)pMemRef->mem, (SMemTable*)pMemRef->imem);
    pMemRef->mem = NULL;
    pMemRef->imem = NULL;
  }
...
static std::vector<STableCheckInfo> createCheckInfoFromTableGroup(STsdbQueryHandle* pQueryHandle,
                                                                   STableGroupInfo* pGroupList, STsdbMeta* pMeta) {
  ...
  for (int32_t i = 0; i < sizeOfGroup; ++i) {
    ...
    for (int32_t j = 0; j < gsize; ++j) {
      ...
      STableCheckInfo info = { .lastKey = pKeyInfo->lastKey, .pTableObj = (STable*)pKeyInfo->pTable };
      ...
      pTableCheckInfo.push_back(info);
  ...
  std::sort(pTableCheckInfo.begin(), pTableCheckInfo.end());
  return pTableCheckInfo;
}

static std::vector<STableCheckInfo> createCheckInfoFromCheckInfo(
    const std::vector<STableCheckInfo> &pTableCheckInfo, TSKEY skey) {
  std::vector<STableCheckInfo> pNew;

  for (const auto& pCheckInfo : pTableCheckInfo) {
...
static STsdbQueryHandle* tsdbQueryTablesImpl(TSDB_REPO_T* tsdb, STsdbQueryCond* pCond, void* qinfo, SMemRef* pMemRef) {
  STsdbQueryHandle* pQueryHandle = (STsdbQueryHandle*)calloc(1, sizeof(STsdbQueryHandle));
  ...
  pQueryHandle->pTsdb       = (STsdbRepo*)tsdb;
  ...
  // allocate buffer in order to load data blocks from file
  pQueryHandle->statis = (SDataStatis*)calloc(pCond->numOfCols, sizeof(SDataStatis));
  if (pQueryHandle->statis == NULL) {
    return nullptr;
  }

  pQueryHandle->pColumns = (SArray*)taosArrayInit(pCond->numOfCols, sizeof(SColumnInfoData));  // todo: use list instead of array?
...
TsdbQueryHandleT* STsdbRepo::queryTables(STsdbQueryCond* pCond, STableGroupInfo* groupList,
  ...
  if (pQueryHandle->pTableCheckInfo.empty()) {
    tsdbCleanupQueryHandle(pQueryHandle);
    terrno = TSDB_CODE_TDB_OUT_OF_MEMORY;
    return NULL;
  }

  tsdbDebug("%p total numOfTable:%" PRIzu " in query, %p", pQueryHandle, pQueryHandle->pTableCheckInfo.size(), pQueryHandle->qinfo);
...
SArray* tsdbGetQueriedTableList(TsdbQueryHandleT *pHandle) {
  ...
  size_t            size = pQueryHandle->pTableCheckInfo.size();
  SArray* res = (SArray*)taosArrayInit(size, POINTER_BYTES);

  for (auto& pCheckInfo : pQueryHandle->pTableCheckInfo) {
...
static bool initTableMemIterator(STsdbQueryHandle* pHandle, STableCheckInfo* pCheckInfo) {
  ...
  SMemTable* pMemT = (SMemTable*)pHandle->pMemRef->mem;
  SMemTable* pIMemT = (SMemTable*)pHandle->pMemRef->imem;
...
static bool hasMoreDataInCache(STsdbQueryHandle* pHandle) {
  ...
  size_t size = pHandle->pTableCheckInfo.size();
  assert(pHandle->activeIndex < size && pHandle->activeIndex >= 0 && size >= 1);
  pHandle->cur.fid = -1;

  STableCheckInfo* pCheckInfo = &pHandle->pTableCheckInfo[pHandle->activeIndex];
  ...
  *numOfBlocks = 0;
  ...
  for (auto& pCheckInfo : pQueryHandle->pTableCheckInfo) {
    ...
    if (pCheckInfo.compSize < (int32_t)compIndex->len) {
      ...
      char* t = (char*)realloc(pCheckInfo.pCompInfo, compIndex->len);
      if (t == NULL) {
        terrno = TSDB_CODE_TDB_OUT_OF_MEMORY;
...
static int32_t doLoadFileDataBlock(STsdbQueryHandle* pQueryHandle, SCompBlock* pBlock, STableCheckInfo* pCheckInfo, int32_t slotIndex) {
  ...
  int16_t* colIds = (int16_t*)pQueryHandle->defaultLoadColumn->pData;
  ...
  if (ASCENDING_TRAVERSE(pQueryHandle->order)) {
    ...
    if (pQueryHandle->window.ekey < pBlock->keyLast || pCheckInfo->lastKey > pBlock->keyFirst) {
      ...
      if (pCheckInfo->lastKey > pBlock->keyFirst) {
        cur->pos =
            binarySearchForKey((char *)pTSCol->cols[0].pData, pBlock->numOfRows, pCheckInfo->lastKey, pQueryHandle->order);
      } else {
        cur->pos = 0;
      }
    ...
    if (pQueryHandle->window.ekey > pBlock->keyFirst || pCheckInfo->lastKey < pBlock->keyLast) {
      ...
      if (pCheckInfo->lastKey < pBlock->keyLast) {
        cur->pos = binarySearchForKey((char *)pTsCol->cols[0].pData, pBlock->numOfRows, pCheckInfo->lastKey, pQueryHandle->order);
      } else {
...
int32_t doCopyRowsFromFileBlock(STsdbQueryHandle* pQueryHandle, int32_t capacity, int32_t numOfRows, int32_t start, int32_t end) {
  ...
  SDataCols* pCols = pQueryHandle->rhelper.pDataCols[0];
  TSKEY* tsArray = (TSKEY*)pCols->cols[0].pData;
...
static void doCheckGeneratedBlockRange(STsdbQueryHandle* pQueryHandle) {
  ...
  if (cur->rows > 0) {
    ...
    SColumnInfoData* pColInfoData = (SColumnInfoData*)taosArrayGet(pQueryHandle->pColumns, 0);
...
int32_t getEndPosInDataBlock(STsdbQueryHandle* pQueryHandle, SDataBlockInfo* pBlockInfo) {
  ...
  if (ASCENDING_TRAVERSE(pQueryHandle->order) && pQueryHandle->window.ekey >= pBlockInfo->window.ekey) {
    ...
    endPos = doBinarySearchKey((char*)pCols->cols[0].pData, pCols->numOfRows, pQueryHandle->window.ekey, order);
    cur->mixBlock = true;
  }
  ...
  if (pCheckInfo->iiter == NULL && pCheckInfo->iter == NULL) {
    ...
    do {
      ...
      if ((key < tsArray[pos] && ASCENDING_TRAVERSE(pQueryHandle->order)) ||
        ...
        int32_t end = doBinarySearchKey((char *)pCols->cols[0].pData, pCols->numOfRows, key, order);
        assert(end != -1);
...
static int32_t createDataBlocksInfo(STsdbQueryHandle* pQueryHandle, int32_t numOfBlocks, int32_t* numOfAllocBlocks) {
  ...
  if (pQueryHandle->allocSize < size) {
    ...
    char* tmp = (char*)realloc(pQueryHandle->pDataBlockInfo, pQueryHandle->allocSize);
    if (tmp == NULL) {
  ...
  *numOfAllocBlocks = numOfBlocks;
  ...
  int32_t numOfTables = pQueryHandle->pTableCheckInfo.size();

  SBlockOrderSupporter sup = {0};
  sup.numOfTables = numOfTables;
  sup.numOfBlocksPerTable = (int32_t*)calloc(1, sizeof(int32_t) * numOfTables);
  sup.blockIndexArray = (int32_t*)calloc(1, sizeof(int32_t) * numOfTables);
  sup.pDataBlockInfo = (STableBlockInfo**)calloc(1, POINTER_BYTES * numOfTables);
  ...
  for (int32_t j = 0; j < numOfTables; ++j) {
    STableCheckInfo* pTableCheck = &pQueryHandle->pTableCheckInfo[j];
    ...
    char* buf = (char*)calloc(1, sizeof(STableBlockInfo) * pTableCheck->numOfBlocks);
    if (buf == NULL) {
...
static bool doHasDataInBuffer(STsdbQueryHandle* pQueryHandle) {
  size_t numOfTables = pQueryHandle->pTableCheckInfo.size();
...
static void changeQueryHandleForInterpQuery(TsdbQueryHandleT pHandle) {
  ...
  int32_t i = 0;
  while(i < numOfTables) {
    STableCheckInfo* pCheckInfo = &pQueryHandle->pTableCheckInfo[i];
  ...
  STableCheckInfo info = pQueryHandle->pTableCheckInfo[i];
  pQueryHandle->pTableCheckInfo.clear();

  info.lastKey = pQueryHandle->window.skey;
  pQueryHandle->pTableCheckInfo.push_back(info);
...
static bool getNeighborRows(STsdbQueryHandle* pQueryHandle) {
  ...
  if (pQueryHandle->cur.win.ekey == pQueryHandle->window.skey) {
    ...
    cond.colList = (SColumnInfo*)calloc(cond.numOfCols, sizeof(SColumnInfo));
    ...
    for(int32_t i = 0; i < cond.numOfCols; ++i) {
      SColumnInfoData* pColInfoData = (SColumnInfoData*)taosArrayGet(pQueryHandle->pColumns, i);
    ...
    if (pSecQueryHandle->pTableCheckInfo.empty()) {
    ...
    size_t si = pSecQueryHandle->pTableCheckInfo.size();

    for (int32_t i = 0; i < numOfCols; ++i) {
      SColumnInfoData* pCol = (SColumnInfoData*)taosArrayGet(pQueryHandle->pColumns, i);
      memcpy((char*)pCol->pData, (char*)pCol->pData + pCol->info.bytes * (pQueryHandle->cur.rows - 1), pCol->info.bytes);

      SColumnInfoData* pCol1 = (SColumnInfoData*)taosArrayGet(pSecQueryHandle->pColumns, i);
      assert(pCol->info.colId == pCol1->info.colId);

      memcpy((char*)pCol->pData + pCol->info.bytes, pCol1->pData, pCol1->info.bytes);
    }

    SColumnInfoData* pTSCol = (SColumnInfoData*)taosArrayGet(pQueryHandle->pColumns, 0);
    ...
    for (int32_t j = 0; j < si; ++j) {
      STableCheckInfo* pCheckInfo = &pQueryHandle->pTableCheckInfo[j];
...
bool tsdbNextDataBlock(TsdbQueryHandleT* pHandle) {
  ...
  if (pQueryHandle->type == TSDB_QUERY_TYPE_EXTERNAL) {
    ...
    if (++pQueryHandle->activeIndex < numOfTables) {
      STableCheckInfo* pCheckInfo = &pQueryHandle->pTableCheckInfo[pQueryHandle->activeIndex];
...
int32_t checkForCachedLastRow(STsdbQueryHandle* pQueryHandle, STableGroupInfo *groupList) {
  ...
  SArray* group = (SArray*)taosArrayGetP(groupList->pGroupList, 0);
  assert(group != NULL);

  STableKeyInfo* pInfo = (STableKeyInfo*)taosArrayGet(group, 0);

  int32_t code = tsdbGetCachedLastRow((STable*)pInfo->pTable, &pRow, &key);
  if (code != TSDB_CODE_SUCCESS) {
...
STimeWindow updateLastrowForEachGroup(STableGroupInfo *groupList) {
  ...
  for(int32_t j = 0; j < numOfGroups; ++j) {
    SArray* pGroup = (SArray*)taosArrayGetP(groupList->pGroupList, j);
    TSKEY   key = TSKEY_INITIAL_VAL;
    ...
    for (int32_t i = 0; i < numOfTables; ++i) {
      ...
      if (pInfo->pTable != keyInfo.pTable) {
        tsdbUnRefTable((STable*)pInfo->pTable);
      }
    }
...
int32_t tsdbRetrieveDataBlockStatisInfo(TsdbQueryHandleT* pQueryHandle, SDataStatis** pBlockStatis) {
  ...
  int16_t* colIds = (int16_t*)pHandle->defaultLoadColumn->pData;
  ...
  for(int32_t i = 1; i < numOfCols; ++i) {
    ...
    SColumnInfo* pColInfo = (SColumnInfo*)taosArrayGet(pHandle->pColumns, i);
      ...
      if (pBlockLoadInfo->slot == pHandle->cur.slot && pBlockLoadInfo->fileGroup->fileId == pHandle->cur.fid &&
        ...
        if (!ASCENDING_TRAVERSE(pHandle->order) && numOfRows < pHandle->outputCapacity) {
          ...
          for(int32_t i = 0; i < reqNumOfCols; ++i) {
            SColumnInfoData* pColInfo = (SColumnInfoData*)taosArrayGet(pHandle->pColumns, i);
            memmove((char*)pColInfo->pData, (char*)pColInfo->pData + emptySize * pColInfo->info.bytes, numOfRows * pColInfo->info.bytes);
          }
        }
...
void filterPrepare(void* expr, void* param) {
  ...
  tQueryInfo* pInfo = (tQueryInfo*)pExpr->_node.info;
  ...
  if (pInfo->optr == TSDB_RELATION_IN) {
    ...
    pInfo->q = (char*)calloc(1, size + TSDB_NCHAR_SIZE + VARSTR_HEADER_SIZE);
...
static int32_t tableGroupComparFn(const void *p1, const void *p2, const void *param) {
  ...
  STable* pTable1 = (STable*)((STableKeyInfo*) p1)->pTable;
  STable* pTable2 = (STable*)((STableKeyInfo*) p2)->pTable;
  ...
  for (int32_t i = 0; i < pTableGroupSupp->numOfCols; ++i) {
    ...
    if (colIndex == TSDB_TBNAME_COLUMN_INDEX) {
      ...
      bytes = pCol->bytes;
      type = pCol->type;
      f1 = (char *)tdGetKVRowValOfCol(pTable1->tagVal, pCol->colId);
      f2 = (char *)tdGetKVRowValOfCol(pTable2->tagVal, pCol->colId);
    }
...
void createTableGroupImpl(SArray* pGroups, SArray* pTableList, size_t numOfTables, TSKEY skey,
  ...
  STable* pTable = (STable*)taosArrayGetP(pTableList, 0);

  SArray* g = (SArray*)taosArrayInit(16, sizeof(STableKeyInfo));
  ...
  for (int32_t i = 1; i < numOfTables; ++i) {
    STable** prev = (STable**)taosArrayGet(pTableList, i - 1);
    STable** p = (STable**)taosArrayGet(pTableList, i);
    ...
    if (ret == 0) {
      ...
      g = (SArray*)taosArrayInit(16, sizeof(STableKeyInfo));
...
SArray* createTableGroup(SArray* pTableList, STSchema* pTagSchema, SColIndex* pCols, int32_t numOfOrderCols, TSKEY skey) {
  ...
  SArray* pTableGroup = (SArray*)taosArrayInit(1, POINTER_BYTES);
  ...
  if (numOfOrderCols == 0 || size == 1) { // no group by tags clause or only one table
    SArray* sa = (SArray*)taosArrayInit(size, sizeof(STableKeyInfo));
    if (sa == NULL) {
    ...
    for(int32_t i = 0; i < size; ++i) {
      STableKeyInfo *pKeyInfo = (STableKeyInfo *)taosArrayGet(pTableList, i);
      assert(((STable*)pKeyInfo->pTable)->type == TSDB_CHILD_TABLE);

      tsdbRefTable((STable*)pKeyInfo->pTable);
...
static bool tableFilterFp(const void* pNode, void* param) {
  ...
  if (pInfo->sch.colId == TSDB_TBNAME_COLUMN_INDEX) {
    ...
    val = (char*)tdGetKVRowValOfCol(pTable->tagVal, pInfo->sch.colId);
  }
...
int32_t STsdbRepo::querySTable(uint64_t uid, TSKEY skey, const char* pTagCond, size_t len,
  ...
  SArray* res = (SArray*)taosArrayInit(8, sizeof(STableKeyInfo));
...
int32_t tsdbGetOneTableGroup(TSDB_REPO_T* tsdb, uint64_t uid, TSKEY startKey, STableGroupInfo* pGroupInfo) {
  ...
  pGroupInfo->pGroupList = (SArray*)taosArrayInit(1, POINTER_BYTES);

  SArray* group = (SArray*)taosArrayInit(1, sizeof(STableKeyInfo));
...
void tsdbCleanupQueryHandle(TsdbQueryHandleT queryHandle) {
  ...
  if (!pQueryHandle->pTableCheckInfo.empty()) {
    size_t size = pQueryHandle->pTableCheckInfo.size();
    for (int32_t i = 0; i < size; ++i) {
      STableCheckInfo* pTableCheckInfo = &pQueryHandle->pTableCheckInfo[i];
...
void tsdbDestroyTableGroup(STableGroupInfo *pGroupList) {
  ...
  for(int32_t i = 0; i < numOfGroup; ++i) {
    SArray* p = (SArray*)taosArrayGetP(pGroupList->pGroupList, i);

    size_t numOfTables = taosArrayGetSize(p);
    for(int32_t j = 0; j < numOfTables; ++j) {
      STable* pTable = (STable*)taosArrayGetP(p, j);
      assert(pTable != NULL);
...
static int32_t setQueryCond(tQueryInfo *queryColInfo, SQueryCond* pCond) {
  ...
  if (optr == TSDB_RELATION_GREATER || optr == TSDB_RELATION_GREATER_EQUAL ||
    ...
    pCond->start = new SEndPoint{};
    pCond->start->optr = queryColInfo->optr;
    pCond->start->v = queryColInfo->q;
  } else if (optr == TSDB_RELATION_LESS || optr == TSDB_RELATION_LESS_EQUAL) {
    pCond->end = new SEndPoint{};
...
static void queryIndexlessColumn(SSkipList* pSkipList, tQueryInfo* pQueryInfo, SArray* res, __result_filter_fn_t filterFp) {
  ...
  while (tSkipListIterNext(iter)) {
    ...
    char *pData = (char*)SL_GET_NODE_DATA(pNode);
...
void getTableListfromSkipList(tExprNode *pExpr, SSkipList *pSkipList, SArray *result, SExprTraverseSupp *param) {
  ...
  if (pLeft->nodeType != TSQL_NODE_EXPR && pRight->nodeType != TSQL_NODE_EXPR) {
    ...
    tQueryInfo *pQueryInfo = (tQueryInfo*)pExpr->_node.info;
