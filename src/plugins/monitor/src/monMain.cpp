...
void monSaveAcctLog(SAcctMonitorObj *pMon) {
  ...
  taos_query_a(tsMonitor.conn, sql, monExecSqlCb, (void *)"account info");
}
...
void monSaveLog(int32_t level, const char *const format, ...) {
  ...
  taos_query_a(tsMonitor.conn, sql, monExecSqlCb, (void *)"log");
}
...
void monExecuteSQL(char *sql) {
  ...
  taos_query_a(tsMonitor.conn, sql, monExecSqlCb, (void*)"sql");
}
