...
void httpCleanupContexts() {
  if (tsHttpServer.contextCache != NULL) {
    SCacheObj *cache = (SCacheObj*)tsHttpServer.contextCache;
    httpInfo("context cache is cleanuping, size:%d", taosHashGetSize(cache->pHashTable));
    taosCacheCleanup((SCacheObj*)tsHttpServer.contextCache);
...
HttpContext *httpCreateContext(int32_t fd) {
  HttpContext *pContext = (HttpContext*)calloc(1, sizeof(HttpContext));
  ...
  HttpContext **ppContext = (HttpContext **)taosCachePut((SCacheObj*)tsHttpServer.contextCache, &handleVal, sizeof(TSDB_CACHE_PTR_TYPE), &pContext,
                                         sizeof(TSDB_CACHE_PTR_TYPE), 3000);
  pContext->ppContext = ppContext;
  httpDebug("context:%p, fd:%d, is created, data:%p", pContext, fd, ppContext);

  // set the ref to 0 
  taosCacheRelease((SCacheObj*)tsHttpServer.contextCache, (void**)&ppContext, false);
...
HttpContext *httpGetContext(void *ptr) {
  ...
  HttpContext **ppContext = (HttpContext **)taosCacheAcquireByKey((SCacheObj*)tsHttpServer.contextCache, &handleVal, sizeof(TSDB_CACHE_PTR_TYPE));
...
void httpReleaseContext(HttpContext *pContext, bool clearRes) {
  ...
  HttpContext **ppContext = (HttpContext **)pContext->ppContext;
  ...
  if (keepAlive) {
    if (httpAlterContextState(pContext, HTTP_CONTEXT_STATE_HANDLING, HTTP_CONTEXT_STATE_READY)) {
      ...
      httpError("context:%p, fd:%d, last state:%s:%d, keepAlive:true, close connect", pContext, pContext->fd,
                httpContextStateStr((HttpContextState)pContext->state), pContext->state);
    }
  } else {
    httpRemoveContextFromEpoll(pContext);
