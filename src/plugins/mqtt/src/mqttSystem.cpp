...
int32_t mqttStartSystem() {
  ...
  tsMqttStatus.sendbuf = (uint8_t*)malloc(MQTT_SEND_BUF_SIZE);
  tsMqttStatus.recvbuf = (uint8_t*)malloc(MQTT_RECV_BUF_SIZE);
  tsMqttIsRuning = true;
...
void mqttPublishCallback(void** unused, struct mqtt_response_publish* published) {
  const char* content = (const char*)published->application_message;
