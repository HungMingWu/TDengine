...
int32_t compareLenPrefixedStr(const void *pLeft, const void *pRight) {
  ...
  if (len1 != len2) {
    ...
    int32_t ret = strncmp((char*)varDataVal(pLeft), (char*)varDataVal(pRight), len1);
    if (ret == 0) {
      return 0;
    } else {
      return ret > 0 ? 1:-1;
...
int32_t compareLenPrefixedWStr(const void *pLeft, const void *pRight) {
  ...
  if (len1 != len2) {
    ...
    int32_t ret = wcsncmp((wchar_t*)varDataVal(pLeft), (wchar_t*)varDataVal(pRight), len1/TSDB_NCHAR_SIZE);
    if (ret == 0) {
      return 0;
    } else {
      return ret > 0 ? 1 : -1;
...
static int32_t compareFindStrInArray(const void* pLeft, const void* pRight) {
  ...
  return taosArraySearchString(arr, (char *)pLeft, taosArrayCompareString) == NULL ? 0 : 1;
}
...
static int32_t compareWStrPatternComp(const void* pLeft, const void* pRight) {
  ...
  int32_t ret = WCSPatternMatch(pattern, (wchar_t*)varDataVal(pLeft), varDataLen(pLeft)/TSDB_NCHAR_SIZE, &pInfo);
  return (ret == TSDB_PATTERN_MATCH) ? 0 : 1;
}
