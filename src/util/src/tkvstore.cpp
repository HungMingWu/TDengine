...
int tdUpdateKVStoreRecord(SKVStore *pStore, uint64_t uid, void *cont, int contLen) {
  ...
  SKVRecord *pRecord = (SKVRecord*)taosHashGet(pStore->map, (void *)&uid, sizeof(uid));
...
static SKVStore *tdNewKVStore(char *fname, iterFunc iFunc, afterFunc aFunc, void *appH) {
  ...
  pStore->aFunc = aFunc;
  pStore->appH = appH;
  pStore->map = taosHashInit(4096, taosGetDefaultHashFunction(TSDB_DATA_TYPE_BIGINT), true, HASH_NO_LOCK);
  if (pStore->map == NULL) {
    terrno = TSDB_CODE_COM_OUT_OF_MEMORY;
    goto _err;
  }
...
static int tdRestoreKVStore(SKVStore *pStore) {
  ...
  pRecord = (SKVRecord*)taosHashIterate(pStore->map, NULL);
  while (pRecord) {
    ...
    pRecord = (SKVRecord*)taosHashIterate(pStore->map, pRecord);
  }
