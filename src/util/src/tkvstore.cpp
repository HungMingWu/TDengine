...
static int       tdInitKVStoreHeader(int fd, const char *fname);
static int       tdEncodeStoreInfo(void **buf, SStoreInfo *pInfo);
static void *    tdDecodeStoreInfo(void *buf, SStoreInfo *pInfo);
static SKVStore *tdNewKVStore(const char *fname, iterFunc iFunc, afterFunc aFunc, void *appH);
static int       tdUpdateKVStoreHeader(int fd, const char *fname, SStoreInfo *pInfo);
static int       tdLoadKVStoreHeader(int fd, const char *fname, SStoreInfo *pInfo, uint32_t *version);
...
SKVStore *tdOpenKVStore(const char *fname, iterFunc iFunc, afterFunc aFunc, void *appH) {
  ...
  pStore->fd = open(pStore->fname.c_str(), O_RDWR);
  if (pStore->fd < 0) {
    uError("failed to open file %s since %s", pStore->fname.c_str(), strerror(errno));
    terrno = TAOS_SYSTEM_ERROR(errno);
    goto _err;
  }

  pStore->sfd = open(pStore->fsnap.c_str(), O_RDONLY);
  if (pStore->sfd < 0) {
    if (errno != ENOENT) {
      uError("failed to open file %s since %s", pStore->fsnap.c_str(), strerror(errno));
      terrno = TAOS_SYSTEM_ERROR(errno);
      goto _err;
    }
  } else {
    uDebug("file %s exists, try to recover the KV store", pStore->fsnap);
    if (tdLoadKVStoreHeader(pStore->sfd, pStore->fsnap.c_str(), &info, &version) < 0) {
      if (terrno != TSDB_CODE_COM_FILE_CORRUPTED) goto _err;
    } else {
      if (version != KVSTORE_FILE_VERSION) {
        uError("file %s version %u is not the same as program version %u, this may cause problem", pStore->fsnap.c_str(),
               version, KVSTORE_FILE_VERSION);
      }

      if (taosFtruncate(pStore->fd, info.size) < 0) {
        uError("failed to truncate %s to %" PRId64 " size since %s", pStore->fname.c_str(), info.size, strerror(errno));
        terrno = TAOS_SYSTEM_ERROR(errno);
        goto _err;
      }

      if (tdUpdateKVStoreHeader(pStore->fd, pStore->fname.c_str(), &info) < 0) goto _err;
      if (fsync(pStore->fd) < 0) {
        uError("failed to fsync file %s since %s", pStore->fname.c_str(), strerror(errno));
        goto _err;
      }
    }
    ...
    close(pStore->sfd);
    pStore->sfd = -1;
    (void)remove(pStore->fsnap.c_str());
  }

  if (tdLoadKVStoreHeader(pStore->fd, pStore->fname.c_str(), &info, &version) < 0) goto _err;
  if (version != KVSTORE_FILE_VERSION) {
    uError("file %s version %u is not the same as program version %u, this may cause problem", pStore->fname.c_str(), version,
...
int tdKVStoreStartCommit(SKVStore *pStore) {
  ...
  pStore->sfd = open(pStore->fsnap.c_str(), O_WRONLY | O_CREAT, 0755);
  if (pStore->sfd < 0) {
    uError("failed to open file %s since %s", pStore->fsnap.c_str(), strerror(errno));
    terrno = TAOS_SYSTEM_ERROR(errno);
    goto _err;
  ...
  if (fsync(pStore->sfd) < 0) {
    uError("failed to fsync file %s since %s", pStore->fsnap.c_str(), strerror(errno));
    terrno = TAOS_SYSTEM_ERROR(errno);
    goto _err;
  }

  if (close(pStore->sfd) < 0) {
    uError("failed to close file %s since %s", pStore->fsnap.c_str(), strerror(errno));
    terrno = TAOS_SYSTEM_ERROR(errno);
    goto _err;
  }
  pStore->sfd = -1;
  ...
  if (lseek(pStore->fd, 0, SEEK_END) < 0) {
    uError("failed to lseek file %s since %s", pStore->fname.c_str(), strerror(errno));
    terrno = TAOS_SYSTEM_ERROR(errno);
    goto _err;
  }
...
int tdUpdateKVStoreRecord(SKVStore *pStore, uint64_t uid, void *cont, int contLen) {
  ...
  if (taosWrite(pStore->fd, buf, tlen) < tlen) {
    uError("failed to write %d bytes to file %s since %s", tlen, pStore->fname.c_str(), strerror(errno));
    terrno = TAOS_SYSTEM_ERROR(errno);
    return -1;
  }

  if (taosWrite(pStore->fd, cont, contLen) < contLen) {
    uError("failed to write %d bytes to file %s since %s", contLen, pStore->fname.c_str(), strerror(errno));
    terrno = TAOS_SYSTEM_ERROR(errno);
    return -1;
  }
  ...
  auto it = pStore->map.find(uid);
  if (it != pStore->map.end()) {  // just to insert
    pStore->info.tombSize += it->second.size;
  } else {
    pStore->info.nRecords++;
  }

  pStore->map[uid] = rInfo;
  uTrace("put uid %" PRIu64 " into kvStore %s", uid, pStore->fname.c_str());
...
int tdDropKVStoreRecord(SKVStore *pStore, uint64_t uid) {
  ...
  auto it = pStore->map.find(uid);
  if (it == pStore->map.end()) {
    uError("failed to drop KV store record with key %" PRIu64 " since not find", uid);
    return -1;
  }

  rInfo.offset = -(it->second.offset);
  rInfo.uid = it->second.uid;
  rInfo.size = it->second.size;
  ...
  pStore->map.erase(uid);
  uDebug("drop uid %" PRIu64 " from KV store %s", uid, pStore->fname.c_str());
...
int tdKVStoreEndCommit(SKVStore *pStore) {
  ...
  if (tdUpdateKVStoreHeader(pStore->fd, pStore->fname.c_str(), &(pStore->info)) < 0) return -1;
  ...
  if (close(pStore->fd) < 0) {
    uError("failed to close file %s since %s", pStore->fname.c_str(), strerror(errno));
    terrno = TAOS_SYSTEM_ERROR(errno);
    return -1;
  }
  pStore->fd = -1;
...
static SKVStore *tdNewKVStore(const char *fname, iterFunc iFunc, afterFunc aFunc, void *appH) {
  ...
  pStore->fname = std::string(fname);
  pStore->fsnap = fmt::format("{}{}", fname, TD_KVSTORE_SNAP_SUFFIX);
  pStore->fnew = fmt::format("{}{}", fname, TD_KVSTORE_NEW_SUFFIX);
...
static int tdRestoreKVStore(SKVStore *pStore) {
  ...
  while (true) {
    ...
    if (tsize < sizeof(SKVRecord)) {
      uError("failed to read %" PRIzu " bytes from file %s at offset %" PRId64 "since %s", sizeof(SKVRecord), pStore->fname.c_str(),
             pStore->info.size, strerror(errno));
      terrno = TAOS_SYSTEM_ERROR(errno);
      goto _err;
    }

    char *pBuf = (char*)tdDecodeKVRecord(tbuf, &rInfo);
    ASSERT(POINTER_DISTANCE(pBuf, tbuf) == sizeof(SKVRecord));
    ASSERT((rInfo.offset > 0) ? (pStore->info.size == rInfo.offset) : true);

    if (rInfo.offset < 0) {
      pStore->map.erase(rInfo.uid);
  ...
  if (buf == NULL) {
    uError("failed to allocate %" PRId64 " bytes in KV store %s", maxBufSize, pStore->fname.c_str());
    terrno = TAOS_SYSTEM_ERROR(errno);
    goto _err;
  }

  for (auto &pair : pStore->map) {
    auto pRecord = pair.second;
    if (lseek(pStore->fd, (off_t)(pRecord.offset + sizeof(SKVRecord)), SEEK_SET) < 0) {
      uError("failed to lseek file %s since %s, offset %" PRId64, pStore->fname.c_str(), strerror(errno), pRecord.offset);
      terrno = TAOS_SYSTEM_ERROR(errno);
      goto _err;
    }

    if (taosRead(pStore->fd, buf, (size_t)pRecord.size) < pRecord.size) {
      uError("failed to read %" PRId64 " bytes from file %s since %s, offset %" PRId64, pRecord.size, pStore->fname.c_str(),
    ...
    if (pStore->iFunc) {
      if ((*pStore->iFunc)(pStore->appH, buf, (int)pRecord.size) < 0) {
        uError("failed to restore record uid %" PRIu64 " in kv store %s at offset %" PRId64 " size %" PRId64
               " since %s",
               pRecord.uid, pStore->fname.c_str(), pRecord.offset, pRecord.size, tstrerror(terrno));
        goto _err;
      }
    }
  }
