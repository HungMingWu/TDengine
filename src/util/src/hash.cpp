...
static FORCE_INLINE void __rd_unlock(void *lock, int32_t type) {
  ...
  taosRUnLockLatch((SRWLatch*)lock);
}
...
static FORCE_INLINE void __wr_unlock(void *lock, int32_t type) {
  ...
  taosWUnLockLatch((SRWLatch*)lock);
}
...
SHashObj *taosHashInit(size_t capacity, _hash_fn_t fn, bool update, SHashLockTypeE type) {
  ...
  if (pHashObj->hashList == NULL) {
    ...
    pHashObj->pMemBlock = (SArray*)taosArrayInit(8, sizeof(void *));

    void *p = calloc(pHashObj->capacity, sizeof(SHashEntry));
    for (int32_t i = 0; i < pHashObj->capacity; ++i) {
      pHashObj->hashList[i] = (SHashEntry *)((char *)p + i * sizeof(SHashEntry));
    }
...
void taosHashTableResize(SHashObj *pHashObj) {
  ...
  pHashObj->hashList = (SHashEntry**)pNewEntryList;

  size_t inc = newSize - pHashObj->capacity;
  void * p = calloc(inc, sizeof(SHashEntry));

  for (int32_t i = 0; i < inc; ++i) {
    pHashObj->hashList[i + pHashObj->capacity] = (SHashEntry *)((char *)p + i * sizeof(SHashEntry));
  }
...
SHashNode *doCreateHashNode(const void *key, size_t keyLen, const void *pData, size_t dsize, uint32_t hashVal) {
  SHashNode *pNewNode = (SHashNode*)calloc(1, sizeof(SHashNode) + keyLen + dsize);
...
void *taosHashIterate(SHashObj *pHashObj, void *p) {
  ...
  SHashNode *pNode = NULL;
  if (p) {
    pNode = (SHashNode*)taosHashReleaseNode(pHashObj, p, &slot);
    if (pNode == NULL) {
      SHashEntry *pe = pHashObj->hashList[slot];
      if (pHashObj->type == HASH_ENTRY_LOCK) {
        taosWUnLockLatch(&pe->latch);
