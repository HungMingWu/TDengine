...
void doAsyncQuery(STscObj* pObj, SSqlObj* pSql, __async_cb_func_t fp, void* param, const char* sqlstr, size_t sqlLen) {
  ...
  pSql->sqlstr = (char*)calloc(1, sqlLen + 1);
  if (pSql->sqlstr == NULL) {
...
void taos_query_a(TAOS *taos, const char *sqlstr, __async_cb_func_t fp, void *param) {
  ...
  if (sqlLen > tsMaxSQLStringLen) {
    ...
    tscQueueAsyncError((void*)fp, param, terrno);
    return;
  }
  ...
  if (pSql == NULL) {
    ...
    tscQueueAsyncError((void*)fp, param, TSDB_CODE_TSC_OUT_OF_MEMORY);
    return;
  }
...
static void tscProcessAsyncError(SSchedMsg *pMsg) {
  __async_cb_func_t fp = (__async_cb_func_t)pMsg->ahandle;
