...
SCond* tsGetSTableQueryCond(STagCond* pTagCond, uint64_t uid) {
  ...
  for (int32_t i = 0; i < size; ++i) {
    SCond* pCond = (SCond*)taosArrayGet(pTagCond->pCond, i);
...
void tsSetSTableQueryCond(STagCond* pTagCond, uint64_t uid, SBufferWriter* bw) {
  ...
  if (pTagCond->pCond == NULL) {
    pTagCond->pCond = (SArray*)taosArrayInit(3, sizeof(SCond));
  }
...
void*  tscDestroyBlockArrayList(SArray* pDataBlockList) {
  ...
  for (int32_t i = 0; i < size; i++) {
    ...
    tscDestroyDataBlock((STableDataBlocks*)d);
  }
...
static void extractTableNameList(SSqlCmd* pCmd, bool freeBlockMap) {
  ...
  if (pCmd->pTableNameList == NULL) {
    pCmd->pTableNameList = (char**)calloc(pCmd->numOfTables, POINTER_BYTES);
  } else {
...
int32_t tscMergeTableDataBlocks(SSqlObj* pSql, bool freeBlockMap) {
  ...
  while(pOneTableBlock) {
    ...
    int32_t ret = tscGetDataBlockFromList((SHashObj*)pVnodeDataBlockHashList, pOneTableBlock->vgId, TSDB_PAYLOAD_SIZE,
                                INSERT_HEAD_SIZE, 0, pOneTableBlock->tableName, pOneTableBlock->pTableMeta, &dataBuf, pVnodeDataBlockList);
    if (ret != TSDB_CODE_SUCCESS) {
      tscError("%p failed to prepare the data block buffer for merging table data, code:%d", pSql, ret);
      taosHashCleanup((SHashObj*)pVnodeDataBlockHashList);
...
SInternalField* tscFieldInfoAppend(SFieldInfo* pFieldInfo, TAOS_FIELD* pField) {
  ...
  info.pSqlExpr = NULL;
  info.pArithExprInfo = NULL;
  info.visible = true;
  info.field = *pField;
  return (SInternalField*)taosArrayPush(pFieldInfo->internalField, &info);
}
...
SInternalField* tscFieldInfoInsert(SFieldInfo* pFieldInfo, int32_t index, TAOS_FIELD* field) {
  ...
  info.visible = true;
  info.field = *field;
  return (SInternalField*)taosArrayInsert(pFieldInfo->internalField, index, &info);
}

void tscFieldInfoUpdateOffset(SQueryInfo* pQueryInfo) {
  size_t numOfExprs = tscSqlExprNumOfExprs(pQueryInfo);
  
  SSqlExpr* pExpr =(SSqlExpr*) taosArrayGetP(pQueryInfo->exprList, 0);
  pExpr->offset = 0;

  for (int32_t i = 1; i < numOfExprs; ++i) {
    SSqlExpr* prev = (SSqlExpr*)taosArrayGetP(pQueryInfo->exprList, i - 1);
    SSqlExpr* p = (SSqlExpr*)taosArrayGetP(pQueryInfo->exprList, i);
...
SInternalField* tscFieldInfoGetInternalField(SFieldInfo* pFieldInfo, int32_t index) {
  ...
  return (SInternalField*)TARRAY_GET_ELEM(pFieldInfo->internalField, index);
}
...
void tscFieldInfoClear(SFieldInfo* pFieldInfo) {
  ...
  for(int32_t i = 0; i < pFieldInfo->numOfOutput; ++i) {
    SInternalField* pInfo = (SInternalField*)taosArrayGet(pFieldInfo->internalField, i);
...
SSqlExpr* tscSqlExprGet(SQueryInfo* pQueryInfo, int32_t index) {
  return (SSqlExpr*)taosArrayGetP(pQueryInfo->exprList, index);
}
...
int32_t tscTagCondCopy(STagCond* dest, const STagCond* src) {
  ...
  dest->pCond = (SArray*)taosArrayInit(s, sizeof(SCond));
  
  for (int32_t i = 0; i < s; ++i) {
    SCond* pCond = (SCond*)taosArrayGet(src->pCond, i);
...
void tscTagCondRelease(STagCond* pTagCond) {
  ...
  if (pTagCond->pCond != NULL) {
    ...
    for (int32_t i = 0; i < s; ++i) {
      SCond* p = (SCond*)taosArrayGet(pTagCond->pCond, i);
      tfree(p->cond);
    }
...
void tscInitQueryInfo(SQueryInfo* pQueryInfo) {
  ...
  pQueryInfo->fieldsInfo.internalField = (SArray*)taosArrayInit(4, sizeof(SInternalField));
...
SArray* tscVgroupTableInfoClone(SArray* pVgroupTables) {
  ...
  size_t num = taosArrayGetSize(pVgroupTables);
  SArray* pa = (SArray*)taosArrayInit(num, sizeof(SVgroupTableInfo));
...
SSqlObj* createSubqueryObj(SSqlObj* pSql, int16_t tableIndex, __async_cb_func_t fp, void* param, int32_t cmd, SSqlObj* pPrevSql) {
  ...
  if (pQueryInfo->fillType != TSDB_FILL_NONE) {
    pNewQueryInfo->fillVal = (int64_t*)malloc(pQueryInfo->fieldsInfo.numOfOutput * sizeof(int64_t));
