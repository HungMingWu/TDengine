...
static void jniGetGlobalMethod(JNIEnv *env) {
  ...
  g_arrayListClass = env->NewGlobalRef((jobject)arrayListClass);
  g_arrayListConstructFp = env->GetMethodID(g_arrayListClass, "<init>", "()V");
  g_arrayListAddFp = env->GetMethodID(g_arrayListClass, "add", "(Ljava/lang/Object;)Z");
  env->DeleteLocalRef(arrayListClass);

  jclass metadataClass = env->FindClass("com/taosdata/jdbc/ColumnMetaData");
  g_metadataClass = env->NewGlobalRef(metadataClass);
  g_metadataConstructFp = env->GetMethodID(g_metadataClass, "<init>", "()V");
  g_metadataColtypeField = env->GetFieldID(g_metadataClass, "colType", "I");
  g_metadataColnameField = env->GetFieldID(g_metadataClass, "colName", "Ljava/lang/String;");
  g_metadataColsizeField = env->GetFieldID(g_metadataClass, "colSize", "I");
  g_metadataColindexField = env->GetFieldID(g_metadataClass, "colIndex", "I");
  env->DeleteLocalRef(metadataClass);

  jclass rowdataClass = env->FindClass("com/taosdata/jdbc/TSDBResultSetRowData");
  g_rowdataClass = env->NewGlobalRef(rowdataClass);
  g_rowdataConstructor = env->GetMethodID(g_rowdataClass, "<init>", "(I)V");
  g_rowdataClearFp = env->GetMethodID(g_rowdataClass, "clear", "()V");
  g_rowdataSetBooleanFp = env->GetMethodID(g_rowdataClass, "setBoolean", "(IZ)V");
  g_rowdataSetByteFp = env->GetMethodID(g_rowdataClass, "setByte", "(IB)V");
  g_rowdataSetShortFp = env->GetMethodID(g_rowdataClass, "setShort", "(IS)V");
  g_rowdataSetIntFp = env->GetMethodID(g_rowdataClass, "setInt", "(II)V");
  g_rowdataSetLongFp = env->GetMethodID(g_rowdataClass, "setLong", "(IJ)V");
  g_rowdataSetFloatFp = env->GetMethodID(g_rowdataClass, "setFloat", "(IF)V");
  g_rowdataSetDoubleFp = env->GetMethodID(g_rowdataClass, "setDouble", "(ID)V");
  g_rowdataSetStringFp = env->GetMethodID(g_rowdataClass, "setString", "(ILjava/lang/String;)V");
  g_rowdataSetTimestampFp = env->GetMethodID(g_rowdataClass, "setTimestamp", "(IJ)V");
  g_rowdataSetByteArrayFp = env->GetMethodID(g_rowdataClass, "setByteArray", "(I[B)V");
  env->DeleteLocalRef(rowdataClass);

  jclass blockdataClass = env->FindClass("com/taosdata/jdbc/TSDBResultSetBlockData");
  jclass g_blockdataClass = env->NewGlobalRef(blockdataClass);
  g_blockdataSetByteArrayFp = env->GetMethodID(g_blockdataClass, "setByteArray", "(II[B)V");
  g_blockdataSetNumOfRowsFp = env->GetMethodID(g_blockdataClass, "setNumOfRows", "(I)V");
  g_blockdataSetNumOfColsFp = env->GetMethodID(g_blockdataClass, "setNumOfCols", "(I)V");
  env->DeleteLocalRef(blockdataClass);
#endif
...
JNIEXPORT void JNICALL Java_com_taosdata_jdbc_TSDBJNIConnector_initImp(JNIEnv *env, jobject jobj, jstring jconfigDir) {
  if (jconfigDir != NULL) {
    const char *confDir = env->GetStringUTFChars(jconfigDir, NULL);
...
JNIEXPORT jint JNICALL Java_com_taosdata_jdbc_TSDBJNIConnector_setOptions(JNIEnv *env, jobject jobj, jint optionIndex,
  ...
  if (optionIndex == TSDB_OPTION_LOCALE) {
    const char *locale = env->GetStringUTFChars(optionValue, NULL);
    ...
    env->ReleaseStringUTFChars(optionValue, locale);
  } else if (optionIndex == TSDB_OPTION_CHARSET) {
    const char *charset = env->GetStringUTFChars(optionValue, NULL);
    ...
    env->ReleaseStringUTFChars(optionValue, charset);
  } else if (optionIndex == TSDB_OPTION_TIMEZONE) {
    const char *tz1 = env->GetStringUTFChars(optionValue, NULL);
    if (tz1 && strlen(tz1) != 0) {
    ...
    env->ReleaseStringUTFChars(optionValue, tz1);
  } else {
...
JNIEXPORT jlong JNICALL Java_com_taosdata_jdbc_TSDBJNIConnector_connectImp(JNIEnv *env, jobject jobj, jstring jhost,
  ...
  if (jdbName != NULL) {
    dbname = env->GetStringUTFChars(jdbName, NULL);
  }
...
JNIEXPORT jlong JNICALL Java_com_taosdata_jdbc_TSDBJNIConnector_executeQueryImp(JNIEnv *env, jobject jobj,
  ...
  jsize len = env->GetArrayLength(jsql);
  ...
  env->GetByteArrayRegion(jsql, 0, len, (jbyte *)str);
  if (env->ExceptionCheck()) {
    // todo handle error
  }

  SSqlObj *pSql = (SSqlObj*)taos_query(tscon, str);
...
JNIEXPORT jstring JNICALL Java_com_taosdata_jdbc_TSDBJNIConnector_getErrMsgImp(JNIEnv *env, jobject jobj, jlong tres) {
  ...
  return env->NewStringUTF((const char *)taos_errstr(pSql));
}
...
JNIEXPORT jint JNICALL Java_com_taosdata_jdbc_TSDBJNIConnector_getSchemaMetaDataImp(JNIEnv *env, jobject jobj,
  ...
  if (num_fields == 0) {
    ...
    for (int i = 0; i < num_fields; ++i) {
      jobject metadataObj = env->NewObject(g_metadataClass, g_metadataConstructFp);
      env->SetIntField(metadataObj, g_metadataColtypeField, fields[i].type);
      env->SetIntField(metadataObj, g_metadataColsizeField, fields[i].bytes);
      env->SetIntField(metadataObj, g_metadataColindexField, i);
      jstring metadataObjColname = env->NewStringUTF(fields[i].name);
      env->SetObjectField(metadataObj, g_metadataColnameField, metadataObjColname);
      env->CallBooleanMethod(arrayListObj, g_arrayListAddFp, metadataObj);
    }
  }
...
jstring jniFromNCharToByteArray(JNIEnv *env, char *nchar, int32_t maxBytes) {
    #if 0
  jbyteArray bytes = env->NewByteArray(maxBytes);
  env->SetByteArrayRegion(bytes, 0, maxBytes, (jbyte *)nchar);
  return bytes;
  #else
  return {};
#endif
  ...
  for (int i = 0; i < numOfFields; i++) {
    ...
    switch (fields[i].type) {
      case TSDB_DATA_TYPE_BOOL:
        env->CallVoidMethod(rowobj, g_rowdataSetBooleanFp, i, (jboolean)(*((char *)row[i]) == 1));
        break;
      case TSDB_DATA_TYPE_TINYINT:
        env->CallVoidMethod(rowobj, g_rowdataSetByteFp, i, (jbyte) * ((int8_t *)row[i]));
        break;
      case TSDB_DATA_TYPE_SMALLINT:
        env->CallVoidMethod(rowobj, g_rowdataSetShortFp, i, (jshort) * ((int16_t *)row[i]));
        break;
      case TSDB_DATA_TYPE_INT:
        env->CallVoidMethod(rowobj, g_rowdataSetIntFp, i, (jint) * (int32_t *)row[i]);
        break;
      case TSDB_DATA_TYPE_BIGINT:
        env->CallVoidMethod(rowobj, g_rowdataSetLongFp, i, (jlong) * ((int64_t *)row[i]));
        break;
      case TSDB_DATA_TYPE_FLOAT: {
        float fv = 0;
        fv = GET_FLOAT_VAL(row[i]);
        env->CallVoidMethod(rowobj, g_rowdataSetFloatFp, i, (jfloat)fv);
      } break;
      case TSDB_DATA_TYPE_DOUBLE: {
        double dv = 0;
        dv = GET_DOUBLE_VAL(row[i]);
        env->CallVoidMethod(rowobj, g_rowdataSetDoubleFp, i, (jdouble)dv);
      } break;
      case TSDB_DATA_TYPE_BINARY: {
        memcpy(tmp, row[i], length[i]);  // handle the case that terminated does not exist
        env->CallVoidMethod(rowobj, g_rowdataSetStringFp, i, env->NewStringUTF(tmp));

        memset(tmp, 0, length[i]);
        break;
      }
      case TSDB_DATA_TYPE_NCHAR: {
        env->CallVoidMethod(rowobj, g_rowdataSetByteArrayFp, i,
                               jniFromNCharToByteArray(env, (char *)row[i], length[i]));
        break;
      }
      case TSDB_DATA_TYPE_TIMESTAMP:
        env->CallVoidMethod(rowobj, g_rowdataSetTimestampFp, i, (jlong) * ((int64_t *)row[i]));
        break;
      default:
        break;
    }
...
JNIEXPORT jint JNICALL Java_com_taosdata_jdbc_TSDBJNIConnector_fetchBlockImp(JNIEnv *env, jobject jobj, jlong con,
  ...
  env->CallVoidMethod(rowobj, g_blockdataSetNumOfRowsFp, (jint)numOfRows);
  env->CallVoidMethod(rowobj, g_blockdataSetNumOfColsFp, (jint)numOfFields);

  for (int i = 0; i < numOfFields; i++) {
    env->CallVoidMethod(rowobj, g_blockdataSetByteArrayFp, i, fields[i].bytes * numOfRows,
...
JNIEXPORT jlong JNICALL Java_com_taosdata_jdbc_TSDBJNIConnector_subscribeImp(JNIEnv *env, jobject jobj, jlong con,
  ...
  if (jtopic != NULL) {
    topic = (char *)env->GetStringUTFChars(jtopic, NULL);
  }
  if (jsql != NULL) {
    sql = (char *)env->GetStringUTFChars(jsql, NULL);
  }
...
JNIEXPORT jstring JNICALL Java_com_taosdata_jdbc_TSDBJNIConnector_getTsCharset(JNIEnv *env, jobject jobj) {
  return env->NewStringUTF((const char *)tsCharset);
}
