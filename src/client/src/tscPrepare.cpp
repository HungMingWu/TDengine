...
static int normalStmtPrepare(STscStmt* stmt) {
  ...
  if (normal->numParams > 0) {
    normal->params = (tVariant*)calloc(normal->numParams, sizeof(tVariant));
    ...
    switch (param->type) {
      ...
      case TSDB_DATA_TYPE_TIMESTAMP: {
        switch (bind->buffer_type) {
          ...
          case TSDB_DATA_TYPE_NCHAR: {
            ...
            if (taosParseTime((char*)bind->buffer, &u.v8, len, 3, tsDaylight) == TSDB_CODE_SUCCESS) {
              break;
            }
            return TSDB_CODE_TSC_INVALID_VALUE;
          } break;
      ...
      case TSDB_DATA_TYPE_NCHAR: {
        switch (bind->buffer_type) {
          case TSDB_DATA_TYPE_NCHAR: {
            int32_t output = 0;
            if (!taosMbsToUcs4((char*)bind->buffer, *bind->length, (char*)varDataVal(data + param->offset), param->bytes - VARSTR_HEADER_SIZE, &output)) {
              return TSDB_CODE_TSC_INVALID_VALUE;
            }
...
static int insertStmtBindParam(STscStmt* stmt, TAOS_BIND* bind) {
  ...
  tscGetDataBlockFromList(pCmd->pTableBlockHashList, pTableMeta->id.uid, TSDB_PAYLOAD_SIZE, sizeof(SSubmitBlk),
                          pTableMeta->tableInfo.rowSize, &pTableMetaInfo->name[0], pTableMeta, &pBlock, NULL);
...
static int insertStmtReset(STscStmt* pStmt) {
  ...
  if (pCmd->batchSize > 2) {
    ...
    for (int32_t i = 0; i < size; ++i) {
      STableDataBlocks* pBlock = (STableDataBlocks* )taosArrayGetP(pCmd->pDataBlocks, i);
...
TAOS_STMT* taos_stmt_init(TAOS* taos) {
  ...
  STscStmt* pStmt = (STscStmt*)calloc(1, sizeof(STscStmt));
  if (pStmt == NULL) {
