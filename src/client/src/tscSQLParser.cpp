...
static int32_t insertResultField(SQueryInfo* pQueryInfo, int32_t outputIndex, SColumnList* pIdList, int16_t bytes,
                                 int8_t type, const char* fieldName, SSqlExpr* pSqlExpr);
...
static int32_t parseOrderbyClause(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, SQuerySQL* pQuerySql, const SSchema* pSchema);
 ...
 */
static int32_t invalidSqlErrMsg(std::string &dstBuffer, const char* errMsg) {
...
int32_t tscToSQLCmd(SSqlObj* pSql, struct SSqlInfo* pInfo) {
  ...
  if (!pInfo->valid || terrno == TSDB_CODE_TSC_SQL_SYNTAX_ERROR) {
    ...
    return tscSQLSyntaxErrMsg(pCmd->payload, NULL, pInfo->pzErrMsg);
  }
  ...
  switch (pInfo->type) {
      ...
      if (pCmd->command == TSDB_SQL_CREATE_USER) {
        ...
        if (pUser->type == TSDB_ALTER_USER_PASSWD) {
          ...
          if (strncasecmp(pPrivilege->z, "super", 5) == 0 && pPrivilege->n == 5) {
            ...
            return invalidSqlErrMsg(pCmd->payload, msg5);
          }
        } else {
    ...
    case TSDB_SQL_CFG_LOCAL: {
      ...
      pCmd->payload = std::string(pDCL->a[0].z, pDCL->a[0].n);
      if (pDCL->nTokens == 2) {
        pCmd->payload += " " + std::string(pDCL->a[1].z, pDCL->a[1].n);
      }
    ...
    default:
      return invalidSqlErrMsg(pCmd->payload, "not support sql expression");
  }
...
int32_t parseIntervalClause(SSqlObj* pSql, SQueryInfo* pQueryInfo, SQuerySQL* pQuerySql) {
  ...
  if (isTopBottomQuery(pQueryInfo)) {
    ...
    if (pQueryInfo->interval.interval > 0 && colId != PRIMARYKEY_TIMESTAMP_COL_INDEX) {
      return invalidSqlErrMsg(pCmd->payload, msg4);
    }
  ...
  uint64_t uid = pQueryInfo->getExpr(0)->uid;
  ...
  SSchema s;
  s.bytes = TSDB_KEYSIZE;
  s.type = TSDB_DATA_TYPE_TIMESTAMP;
  s.colId = PRIMARYKEY_TIMESTAMP_COL_INDEX;
...
//  const char* msg4 = "sliding not support yet in ordinary query";
  ...
  if (pQueryInfo->interval.sliding < tsMinSlidingTime) {
    return invalidSqlErrMsg(pCmd->payload, msg0);
  }
...
static bool validateTableColumnInfo(SArray* pFieldList, SSqlCmd* pCmd) {
  ...
  if (numOfCols <= 1 || numOfCols > TSDB_MAX_COLUMNS) {
    invalidSqlErrMsg(pCmd->payload, msg);
    return false;
  }

  // first column must be timestamp
  TAOS_FIELD* pField = (TAOS_FIELD*)taosArrayGet(pFieldList, 0);
  ...
  for (int32_t i = 0; i < numOfCols; ++i) {
    pField = (TAOS_FIELD*)taosArrayGet(pFieldList, i);

    if (pField->bytes == 0) {
      invalidSqlErrMsg(pCmd->payload, msg5);
      return false;
    }

    if (!isValidDataType(pField->type)) {
      invalidSqlErrMsg(pCmd->payload, msg4);
      return false;
    }
    ...
    if (validateColumnName(pField->name) != TSDB_CODE_SUCCESS) {
      invalidSqlErrMsg(pCmd->payload, msg6);
      return false;
    }

    // field name must be unique
    if (has(pFieldList, i + 1, pField->name) == true) {
      invalidSqlErrMsg(pCmd->payload, msg3);
      return false;
    }
  ...
  if (nLen > TSDB_MAX_BYTES_PER_ROW) {
    invalidSqlErrMsg(pCmd->payload, msg2);
    return false;
  }
...
static bool validateTagParams(SArray* pTagsList, SArray* pFieldList, SSqlCmd* pCmd) {
  ...
  for (int32_t i = 0; i < numOfTags; ++i) {
    TAOS_FIELD* p = (TAOS_FIELD*)taosArrayGet(pTagsList, i);
    ...
    if ((p->type == TSDB_DATA_TYPE_BINARY && p->bytes <= 0) ||
      ...
      invalidSqlErrMsg(pCmd->payload, msg7);
      return false;
    }
...
bool validateOneTags(SSqlCmd* pCmd, TAOS_FIELD* pTagField) {
  ...
  int32_t numOfCols = pTableMeta->numOfColumns();
...
static bool has(SArray* pFieldList, int32_t startIdx, const char* name) {
  ...
  for (int32_t j = startIdx; j < numOfCols; ++j) {
    TAOS_FIELD* field = (TAOS_FIELD*)taosArrayGet(pFieldList, j);
...
static int32_t handleArithmeticExpr(SSqlCmd* pCmd, int32_t clauseIndex, int32_t exprIndex, tSqlExprItem* pItem) {
  ...
  if (arithmeticType == NORMAL_ARITHMETIC) {
    ...
    SArray* colList = (SArray*)taosArrayInit(10, sizeof(SColIndex));
    ...
    for(int32_t k = 0; k < numOfNode; ++k) {
      SColIndex* pIndex = (SColIndex*)taosArrayGet(colList, k);
    ...
    if (pInfo->pSqlExpr == NULL) {
      ...
      if (ret != TSDB_CODE_SUCCESS) {
        ...
        return invalidSqlErrMsg(pCmd->payload, "invalid expression in select clause");
      }
...
static void addProjectQueryCol(SQueryInfo* pQueryInfo, int32_t startPos, SColumnIndex* pIndex, tSqlExprItem* pItem) {
  ...
  const SSchema* pSchema = pTableMeta->getColumnSchema(pIndex->columnIndex);

  const char* colName = (pItem->aliasName == NULL) ? pSchema->name : pItem->aliasName;
  ...
  if (pIndex->columnIndex == TSDB_TBNAME_COLUMN_INDEX || pIndex->columnIndex == TSDB_UD_COLUMN_INDEX ||
      pIndex->columnIndex >= pTableMeta->numOfColumns()) {
    ids.num = 0;
  }
...
static void addPrimaryTsColIntoResult(SQueryInfo* pQueryInfo) {
  ...
  STableMetaInfo* pTableMetaInfo = tscGetMetaInfo(pQueryInfo, 0);
  const SSchema*        pSchema = pTableMetaInfo->pTableMeta->getColumnSchema(PRIMARYKEY_TIMESTAMP_COL_INDEX);
...
int32_t insertResultField(SQueryInfo* pQueryInfo, int32_t outputIndex, SColumnList* pIdList, int16_t bytes,
  ...
  for (int32_t i = 0; i < pIdList->num; ++i) {
    ...
    int32_t numOfCols = pTableMetaInfo->pTableMeta->numOfColumns();
...
SSqlExpr* doAddProjectCol(SQueryInfo* pQueryInfo, int32_t colIndex, int32_t tableIndex) {
  ...
  const SSchema* pSchema = pTableMeta->getColumnSchema(colIndex);

  int16_t functionId = (int16_t)((colIndex >= numOfCols) ? TSDB_FUNC_TAGPRJ : TSDB_FUNC_PRJ);
  SColumnIndex index = {.tableIndex = tableIndex,};
  
  if (functionId == TSDB_FUNC_TAGPRJ) {
    index.columnIndex = colIndex - pTableMeta->numOfColumns();
...
SSqlExpr* tscAddSpecialColumnForSelect(SQueryInfo* pQueryInfo, int32_t outputColIndex, int16_t functionId,
                                  SColumnIndex* pIndex, const SSchema* pColSchema, int16_t flag) {
  int16_t colId = getNewResColId(pQueryInfo);
...
int32_t addProjectionExprAndResultField(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, tSqlExprItem* pItem) {
  ...
  if (optr == TK_ALL) {  // project on all fields
    ...
    if (index.columnIndex == TSDB_TBNAME_COLUMN_INDEX) {
      ...
      if (index.columnIndex >= pTableMeta->numOfColumns() && UTIL_TABLE_IS_NORMAL_TABLE(pTableMetaInfo)) {
        return invalidSqlErrMsg(pCmd->payload, msg1);
      }
...
static int32_t setExprInfoForFunctions(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, const SSchema* pSchema, SConvertFunc cvtFunc,
...
int32_t addExprAndResultField(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, int32_t colIndex, tSqlExprItem* pItem, bool finalResult) {
  ...
  switch (optr) {
    case TK_COUNT: {
      ...
      if (pItem->pNode->pParam != NULL) {
        ...
        if (sqlOptr == TK_ALL) {
          ...
          bool isTag = false;
          if (index.columnIndex >= pTableMetaInfo->pTableMeta->numOfColumns() || index.columnIndex == TSDB_TBNAME_COLUMN_INDEX) {
      ...
      if (index.tableIndex < pTableMetaInfo->pTableMeta->numOfColumns()) {
        tscInsertPrimaryTSSourceColumn(pQueryInfo, &index);
      }
    ...
    case TK_LEASTSQUARES: {
      ...
      if (index.columnIndex == TSDB_TBNAME_COLUMN_INDEX) {
        return invalidSqlErrMsg(pCmd->payload, msg6);
      }

      // 2. check if sql function can be applied on this column data type
      pTableMetaInfo = tscGetMetaInfo(pQueryInfo, index.tableIndex);
      const SSchema* pSchema = pTableMetaInfo->pTableMeta->getColumnSchema(index.columnIndex);
      ...
      if (index.columnIndex >= pTableMetaInfo->pTableMeta->numOfColumns()) {
        return invalidSqlErrMsg(pCmd->payload, msg6);
      }
    ...
    case TK_INTERP: {
      ...
      int16_t functionID = 0;
      if (convertFunctionId(optr, &functionID) != TSDB_CODE_SUCCESS) {
        return invalidSqlErrMsg(pCmd->payload, msg9);
      }
      ...
      if (!requireAllFields) {
        ...
        if (pItem->pNode->pParam->nExpr > 1 && (pItem->aliasName != NULL && strlen(pItem->aliasName) > 0)) {
          return invalidSqlErrMsg(pCmd->payload, msg8);
        }
        ...
        for (int32_t i = 0; i < pItem->pNode->pParam->nExpr; ++i) {
          ...
          if (pParamElem->pNode->nSQLOptr == TK_ALL) { // select table.*
            ...
            for (int32_t j = 0; j < pTableMetaInfo->pTableMeta->numOfColumns(); ++j) {
              index.columnIndex = j;
              SStrToken t;
              t.n = (uint32_t)strnlen(pSchema[j].name, TSDB_COL_NAME_LEN);
              t.z = (char *)pSchema[j].name;
            ...
            pTableMetaInfo = (STableMetaInfo*)tscGetMetaInfo(pQueryInfo, index.tableIndex);

            // functions can not be applied to tags
            if ((index.columnIndex >= pTableMetaInfo->pTableMeta->numOfColumns()) || (index.columnIndex < 0)) {
              return invalidSqlErrMsg(pCmd->payload, msg6);
            }
            ...
            if (optr == TK_LAST) {  // todo refactor
              ...
              if (!pGroupBy->columnInfo.empty()) {
                for (const auto& col : pGroupBy->columnInfo) {
                  if (!TSDB_COL_IS_TAG(col.flag) && col.colIndex < pTableMetaInfo->pTableMeta->numOfColumns()) { // group by normal columns
                    SSqlExpr* pExpr = pQueryInfo->getExpr(colIndex + i);
                    pExpr->numOfParams = 1;
        ...
        for (int32_t j = 0; j < pQueryInfo->numOfTables; ++j) {
          ...
          for (int32_t i = 0; i < pTableMetaInfo->pTableMeta->numOfColumns(); ++i) {
            SColumnIndex index = {.tableIndex = j, .columnIndex = i};

            char      name[TSDB_COL_NAME_LEN] = {0};
            SStrToken t;
            t.z = (char *)pSchema[i].name;
            t.n = (uint32_t)strnlen(pSchema[i].name, TSDB_COL_NAME_LEN);
            setResultColName(name, pItem, cvtFunc.originFuncId, &t, true);
          ...
          numOfFields += pTableMetaInfo->pTableMeta->numOfColumns();
        }
...
static int16_t doGetColumnIndex(SQueryInfo* pQueryInfo, int32_t index, SStrToken* pToken) {
  ...
  int32_t  numOfCols = pTableMeta->numOfColumns() + pTableMeta->numOfTags();
  const SSchema* pSchema = pTableMeta->getSchema();
...
int32_t setShowInfo(SSqlObj* pSql, struct SSqlInfo* pInfo) {
  ...
  if (showType == TSDB_MGMT_TABLE_TABLE || showType == TSDB_MGMT_TABLE_METRIC || showType == TSDB_MGMT_TABLE_VGROUP) {
    ...
    if (pPattern->type != 0) {
      ...
      if (!tscValidateTableNameLength(pCmd->payload.length())) {
        return invalidSqlErrMsg(pCmd->payload, msg2);
      }
    }
  } else if (showType == TSDB_MGMT_TABLE_VNODES) {
    if (pShowInfo->prefix.type == 0) {
      return invalidSqlErrMsg(pCmd->payload, "No specified ip of dnode");
    }
...
int32_t setKillInfo(SSqlObj* pSql, struct SSqlInfo* pInfo, int32_t killType) {
  ...
  pCmd->payload = std::string(idStr->z, idStr->n);
...
int32_t tscTansformSQLFuncForSTableQuery(SQueryInfo* pQueryInfo) {
  ...
  for (int32_t k = 0; k < size; ++k) {
    SSqlExpr*   pExpr = pQueryInfo->getExpr(k);
    int16_t functionId = aAggs[pExpr->functionId].stableFuncId;

    int32_t colIndex = pExpr->colInfo.colIndex;
    const SSchema* pSrcSchema = pTableMetaInfo->pTableMeta->getColumnSchema(colIndex);
...
void tscRestoreSQLFuncForSTableQuery(SQueryInfo* pQueryInfo) {
  ...
  size_t size = tscSqlExprNumOfExprs(pQueryInfo);
  for (int32_t i = 0; i < size; ++i) {
    SSqlExpr*   pExpr = pQueryInfo->getExpr(i);
    const SSchema * pSchema = pTableMetaInfo->pTableMeta->getColumnSchema(pExpr->colInfo.colIndex);
...
bool hasUnsupportFunctionsForSTableQuery(SSqlCmd* pCmd, SQueryInfo* pQueryInfo) {
  ...
  if (tscIsTWAQuery(pQueryInfo)) {
    ...
    if (pQueryInfo->groupbyExpr.columnInfo.size() != 1) {
      invalidSqlErrMsg(pCmd->payload, msg2);
      return true;
    } else {
      const auto &col = pQueryInfo->groupbyExpr.columnInfo[0];
      if (col.colIndex != TSDB_TBNAME_COLUMN_INDEX) {
        invalidSqlErrMsg(pCmd->payload, msg2);
        return true;
      }
    }
...
static bool functionCompatibleCheck(SQueryInfo* pQueryInfo, bool joinQuery, bool intervalQuery) {
  ...
  const SSqlExpr* pExpr = pQueryInfo->getExpr(startIdx);
  int32_t functionID = pExpr->functionId;
  ...
  int32_t factor = functionCompatList[pQueryInfo->getExpr(startIdx)->functionId];

  if (pQueryInfo->getExpr(0)->functionId == TSDB_FUNC_LAST_ROW && (joinQuery || intervalQuery)) {
    return false;
  }
  ...
  for (int32_t i = startIdx + 1; i < size; ++i) {
    const SSqlExpr* pExpr1 = pQueryInfo->getExpr(i);
...
int32_t parseGroupbyClause(SQueryInfo* pQueryInfo, SArray* pList, SSqlCmd* pCmd) {
  ...
  for (int32_t i = 0; i < num; ++i) {
    tVariantListItem * pItem = (tVariantListItem*)taosArrayGet(pList, i);
    ...
    if (index.columnIndex == TSDB_TBNAME_COLUMN_INDEX) {
      ...
      pSchema = pTableMeta->getColumnSchema(index.columnIndex);
    }
    ...
    if (groupTag) {
      ...
      SColIndex colIndex;
      colIndex.colIndex = relIndex;
      colIndex.flag = TSDB_COL_TAG;
      colIndex.colId = pSchema->colId;
      pGroupExpr->columnInfo.push_back(colIndex);
...
static int32_t doExtractColumnFilterInfo(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, SColumnFilterInfo* pColumnFilter,
  ...
  const SSchema* pSchema = pTableMetaInfo->pTableMeta->getColumnSchema(columnIndex->columnIndex);
...
static int32_t getTablenameCond(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, tSQLExpr* pTableCond, std::string &sb) {
  ...
  if (ret != TSDB_CODE_SUCCESS) {
    invalidSqlErrMsg(pCmd->payload, msg0);
  }
...
static int32_t getJoinCondInfo(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, tSQLExpr* pExpr) {
  ...
  STableMetaInfo* pTableMetaInfo = tscGetMetaInfo(pQueryInfo, index.tableIndex);
  const SSchema*        pTagSchema1 = pTableMetaInfo->pTableMeta->getColumnSchema(index.columnIndex);
  ...
  pTableMetaInfo = tscGetMetaInfo(pQueryInfo, index.tableIndex);
  const SSchema* pTagSchema2 = pTableMetaInfo->pTableMeta->getColumnSchema(index.columnIndex);
...
static int32_t validateSQLExpr(SSqlCmd* pCmd, tSQLExpr* pExpr, SQueryInfo* pQueryInfo, SColumnList* pList,
  ...
  if (pExpr->nSQLOptr == TK_ID) {
    ...
    const SSchema*    pSchema = pTableMeta->getSchema() + index.columnIndex;
    ...
    for(int32_t i = 0; i < inc; ++i) {
      const SSqlExpr* p1 = pQueryInfo->getExpr(i + outputIndex);
...
static bool validateJoinExprNode(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, tSQLExpr* pExpr, SColumnIndex* pLeftIndex) {
  ...
  const SSchema*        pLeftSchema = pLeftMeterMeta->pTableMeta->getSchema();
  ...
  const SSchema*        pRightSchema = pRightMeterMeta->pTableMeta->getSchema();
...
static int32_t setExprToCond(tSQLExpr** parent, tSQLExpr* pExpr, const char* msg, int32_t parentOptr, std::string &msgBuf) {
  if (*parent != NULL) {
...
static int32_t handleExprInQueryCond(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, tSQLExpr** pExpr, SCondExpr* pCondExpr,
  ...
  if (index.columnIndex == PRIMARYKEY_TIMESTAMP_COL_INDEX) {  // query on time range
    ...
    *type = TSQL_EXPR_TS;
  } else if (index.columnIndex >= pTableMeta->numOfColumns() || index.columnIndex == TSDB_TBNAME_COLUMN_INDEX) {
...
static int32_t setTableCondForSTableQuery(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, const char* account,
  ...
  if (pExpr->nSQLOptr == TK_LIKE) {
    pQueryInfo->tagCond.tbnameCond.cond = sb;
  ...
  dbToken.z = name;
  dbToken.n = (uint32_t)strlen(name);
  ...
  pQueryInfo->tagCond.tbnameCond.cond = sb1;
...
static void doAddJoinTagsColumnsIntoTagList(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, SCondExpr* pCondExpr) {
  ...
  if (QUERY_IS_JOIN_QUERY(pQueryInfo->type) && UTIL_TABLE_IS_SUPER_TABLE(pTableMetaInfo)) {
    ...
    index.columnIndex = index.columnIndex - pTableMetaInfo->pTableMeta->numOfColumns();
    tscColumnListInsert(pTableMetaInfo->tagColList, &index);
...
static int32_t getTagQueryCondExpr(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, SCondExpr* pCondExpr, tSQLExpr** pExpr) {
  ...
  for (int32_t i = 0; i < pQueryInfo->numOfTables; ++i) {
    ...
    for(int32_t j = 0; j < num; ++j) {
      SColIndex* pIndex = (SColIndex*)taosArrayGet(colList, j);
    ...
    taosArrayDestroy(colList);
    if (!pQueryInfo->tagCond.pCond.empty() > 0 && !UTIL_TABLE_IS_SUPER_TABLE(pTableMetaInfo)) {
      return invalidSqlErrMsg(pCmd->payload, "filter on tag not supported for normal table");
    }
  }
...
int32_t parseWhereClause(SQueryInfo* pQueryInfo, tSQLExpr** pExpr, SSqlObj* pSql) {
  ...
  if ((*pExpr)->pLeft == NULL || (*pExpr)->pRight == NULL) {
    return invalidSqlErrMsg(pSql->cmd.payload, msg1);
  }
  ...
  if (!validateFilterExpr(pQueryInfo)) {
    return invalidSqlErrMsg(pSql->cmd.payload, msg2);
  }
...
int32_t getTimeRange(STimeWindow* win, tSQLExpr* pRight, int32_t optr, int16_t timePrecision) {
  ...
  if (pRight->val.nType == TSDB_DATA_TYPE_BINARY) {
    ...
    if (seg != NULL) {
      ...
      SStrToken token;
      token.z = pRight->val.pz;
      token.n = pRight->val.nLen;
      token.type = TK_ID;
...
int32_t parseFillClause(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, SQuerySQL* pQuerySQL) {
  ...
  tVariantListItem* pItem = (tVariantListItem*)taosArrayGet(pFillToken, 0);
  ...
  if (pQueryInfo->fillVal == NULL) {
    pQueryInfo->fillVal = (int64_t*)calloc(size, sizeof(int64_t));
    if (pQueryInfo->fillVal == NULL) {
  ...
  if (strncasecmp(pItem->pVar.pz, "none", 4) == 0 && pItem->pVar.nLen == 4) {
    ...
    for (int32_t i = startPos; i < numOfFillVal; ++i, ++j) {
      ...
      tVariant* p = (tVariant*)taosArrayGet(pFillToken, j);
...
int32_t parseOrderbyClause(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, SQuerySQL* pQuerySql, const SSchema* pSchema) {
  ...
  tVariant* pVar = (tVariant*)taosArrayGet(pSortorder, 0);
  ...
  if (UTIL_TABLE_IS_SUPER_TABLE(pTableMetaInfo)) {  // super table query
    ...
    if (index.columnIndex >= pTableMetaInfo->pTableMeta->numOfColumns()) {
      int32_t relTagIndex = index.columnIndex - pTableMetaInfo->pTableMeta->numOfColumns();
    ...
    if (s == 1) {
      if (orderByTags) {
        pQueryInfo->groupbyExpr.orderIndex = index.columnIndex - pTableMetaInfo->pTableMeta->numOfColumns();

        tVariantListItem* p1 = (tVariantListItem*)taosArrayGet(pQuerySql->pSortOrder, 0);
        pQueryInfo->groupbyExpr.orderType = p1->sortOrder;
      } else if (isTopBottomQuery(pQueryInfo)) {
        /* order of top/bottom query in interval is not valid  */
        const SSqlExpr* pExpr = pQueryInfo->getExpr(0);
        assert(pExpr->functionId == TSDB_FUNC_TS);

        pExpr = pQueryInfo->getExpr(1);
    ...
    if (s == 2) {
      tVariantListItem *pItem = (tVariantListItem*)taosArrayGet(pQuerySql->pSortOrder, 0);
      if (orderByTags) {
        pQueryInfo->groupbyExpr.orderIndex = index.columnIndex - pTableMetaInfo->pTableMeta->numOfColumns();
      ...
      pItem = (tVariantListItem*)taosArrayGet(pQuerySql->pSortOrder, 1);
      ...
      if (index.columnIndex != PRIMARYKEY_TIMESTAMP_COL_INDEX) {
        ...
        tVariantListItem* p1 = (tVariantListItem*)taosArrayGet(pSortorder, 1);
        pQueryInfo->order.order = p1->sortOrder;
        pQueryInfo->order.orderColId = PRIMARYKEY_TIMESTAMP_COL_INDEX;
      }
    }
...
int32_t setAlterTableInfo(SSqlObj* pSql, struct SSqlInfo* pInfo) {
  ...
  if (pAlterSQL->type == TSDB_ALTER_TABLE_ADD_TAG_COLUMN) {
    ...
    TAOS_FIELD* p = (TAOS_FIELD*)taosArrayGet(pFieldList, 0);
    ...
    tVariantListItem* pItem = (tVariantListItem*)taosArrayGet(pAlterSQL->varList, 0);
    ...
    SColumnIndex index = COLUMN_INDEX_INITIALIZER;
    SStrToken    name;
    name.z = pItem->pVar.pz;
    name.n = pItem->pVar.nLen;
    name.type = TK_STRING;
    ...
    if (index.columnIndex < numOfCols) {
      return invalidSqlErrMsg(pCmd->payload, msg10);
    } else if (index.columnIndex == numOfCols) {
      return invalidSqlErrMsg(pCmd->payload, msg11);
    }
    ...
    tVariantListItem* pSrcItem = (tVariantListItem*)taosArrayGet(pAlterSQL->varList, 0);
    tVariantListItem* pDstItem = (tVariantListItem*)taosArrayGet(pAlterSQL->varList, 1);
    ...
    SStrToken srcToken;
    srcToken.z = pSrcItem->pVar.pz;
    srcToken.n = pSrcItem->pVar.nLen;
    srcToken.type = TK_STRING;
    if (getColumnIndexByName(pCmd, &srcToken, pQueryInfo, &srcIndex) != TSDB_CODE_SUCCESS) {
      return invalidSqlErrMsg(pCmd->payload, msg17);
    }

    SStrToken destToken;
    destToken.z = pDstItem->pVar.pz;
    destToken.n = pDstItem->pVar.nLen;
    destToken.type = TK_STRING;
    if (getColumnIndexByName(pCmd, &destToken, pQueryInfo, &destIndex) == TSDB_CODE_SUCCESS) {
      return invalidSqlErrMsg(pCmd->payload, msg19);
    }

    tVariantListItem* pItem = (tVariantListItem*)taosArrayGet(pVarList, 0);
    ...
    SArray* pVarList = pAlterSQL->varList;
    tVariantListItem* item = (tVariantListItem*)taosArrayGet(pVarList, 0);
    int16_t           numOfTags = pTableMeta->numOfTags();

    SColumnIndex columnIndex = COLUMN_INDEX_INITIALIZER;
    SStrToken    name;
    name.type = TK_STRING;
    name.z = item->pVar.pz;
    name.n = item->pVar.nLen;
    if (getColumnIndexByName(pCmd, &name, pQueryInfo, &columnIndex) != TSDB_CODE_SUCCESS) {
      return TSDB_CODE_TSC_INVALID_SQL;
    }

    if (columnIndex.columnIndex < pTableMeta->numOfColumns()) {
      return invalidSqlErrMsg(pCmd->payload, msg12);
    }

    tVariantListItem* pItem = (tVariantListItem*)taosArrayGet(pVarList, 1);
    const SSchema*          pTagsSchema = pTableMetaInfo->pTableMeta->getColumnSchema(columnIndex.columnIndex);
    pAlterSQL->tagData.data = (char*)calloc(1, pTagsSchema->bytes * TSDB_NCHAR_SIZE + VARSTR_HEADER_SIZE);

    if (tVariantDump(&pItem->pVar, pAlterSQL->tagData.data, pTagsSchema->type, true) != TSDB_CODE_SUCCESS) {
      return invalidSqlErrMsg(pCmd->payload, msg13);
    }
    ...
    if ((pTagsSchema->type == TSDB_DATA_TYPE_BINARY || pTagsSchema->type == TSDB_DATA_TYPE_NCHAR) &&
      ...
      return invalidSqlErrMsg(pCmd->payload, msg14);
    }
    ...
    SUpdateTableTagValMsg* pUpdateMsg = (SUpdateTableTagValMsg*)(&pCmd->payload[0]);
    ...
    const SSchema* pTagCols = pTableMeta->getTagSchema();
    ...
    if (size > 1) {
      return invalidSqlErrMsg(pCmd->payload, msg16);
    }
    ...
    if (columnIndex.columnIndex == PRIMARYKEY_TIMESTAMP_COL_INDEX) {
      return invalidSqlErrMsg(pCmd->payload, msg18);
    }
...
int32_t validateSqlFunctionInStreamSql(SSqlCmd* pCmd, SQueryInfo* pQueryInfo) {
  ...
  size_t size = tscSqlExprNumOfExprs(pQueryInfo);
  for (int32_t i = 0; i < size; ++i) {
    int32_t functId = pQueryInfo->getExpr(i)->functionId;
  ...
  for (int32_t k = 0; k < size; ++k) {
    ...
    if (pExpr->functionId == TSDB_FUNC_PRJ && pExpr->colInfo.colId == PRIMARYKEY_TIMESTAMP_COL_INDEX) {
      ...
      for (int32_t j = 0; j < size; ++j) {
        const SSqlExpr* pEx = pQueryInfo->getExpr(j);
...
static int32_t setKeepOption(SSqlCmd* pCmd, SCreateDbMsg* pMsg, SCreateDBInfo* pCreateDb) {
  ...
  if (pKeep != NULL) {
    ...
    tVariantListItem* p0 = (tVariantListItem*)taosArrayGet(pKeep, 0);
    switch (s) {
      case 1: {
        ...
        break;
      case 2: {
        tVariantListItem* p1 = (tVariantListItem*)taosArrayGet(pKeep, 1);
        pMsg->daysToKeep = htonl((int32_t)p0->pVar.i64);
      ...
      case 3: {
        tVariantListItem* p1 = (tVariantListItem*)taosArrayGet(pKeep, 1);
        tVariantListItem* p2 = (tVariantListItem*)taosArrayGet(pKeep, 2);
      ...
      default: { return invalidSqlErrMsg(pCmd->payload, msg); }
    }
  }
...
int32_t parseCreateDBOptions(SSqlCmd* pCmd, SCreateDBInfo* pCreateDbSql) {
  SCreateDbMsg* pMsg = (SCreateDbMsg *)(&pCmd->payload[0]);
...
void addGroupInfoForSubquery(SSqlObj* pParentObj, SSqlObj* pSql, int32_t subClauseIndex, int32_t tableIndex) {
  ...
  if (!pParentQueryInfo->groupbyExpr.columnInfo.empty()) {
    SQueryInfo* pQueryInfo = tscGetQueryInfoDetail(&pSql->cmd, subClauseIndex);
    SSqlExpr* pExpr = NULL;

    size_t size = taosArrayGetSize(pQueryInfo->exprList);
    if (size > 0) {
      pExpr = pQueryInfo->getExpr((int32_t)size - 1);
    }
    ...
    if (pExpr == NULL || pExpr->functionId != TSDB_FUNC_TAG) {
      ...
      SSchema* pTagSchema = pTableMetaInfo->pTableMeta->getColumnSchemaById(colId);
      ...
      auto &col = pQueryInfo->groupbyExpr.columnInfo[0];
      col.colIndex = relIndex;
...
void doAddGroupColumnForSubquery(SQueryInfo* pQueryInfo, const SColIndex &col) {
  ...
  const SSchema*     pSchema = pTableMetaInfo->pTableMeta->getColumnSchema(col.colIndex);
  SColumnIndex colIndex = {.tableIndex = 0, .columnIndex = col.colIndex};
...
//todo is 0??
  ...
  for (int32_t i = 0; i < size; ++i) {
    ...
    if ((pExpr->functionId != TSDB_FUNC_TAG_DUMMY && pExpr->functionId != TSDB_FUNC_TS_DUMMY) &&
      ...
      const SSchema* pColSchema = &pSchema[pExpr->colInfo.colIndex];
...
static int32_t doUpdateSqlFunctionForColPrj(SQueryInfo* pQueryInfo) {
  ...
  for (int32_t i = 0; i < size; ++i) {
    ...
    if (pExpr->functionId == TSDB_FUNC_PRJ && (!TSDB_COL_IS_UD_COL(pExpr->colInfo.flag) && (pExpr->colInfo.colId != PRIMARYKEY_TIMESTAMP_COL_INDEX))) {
      ...
      for (const auto &col : pQueryInfo->groupbyExpr.columnInfo) {
        if (pExpr->colInfo.colId == col.colId) {
          qualifiedCol = true;
...
static bool tagColumnInGroupby(SSqlGroupbyExpr* pGroupbyExpr, int16_t columnId) {
  for (const auto &col : pGroupbyExpr->columnInfo) {
    if (columnId == col.colId && TSDB_COL_IS_TAG(col.flag )) {
      return true;
    }
  }
...
static int32_t checkUpdateTagPrjFunctions(SQueryInfo* pQueryInfo, SSqlCmd* pCmd) {
  ...
  for (int32_t i = 0; i < numOfExprs; ++i) {
    SSqlExpr* pExpr = (SSqlExpr*)taosArrayGetP(pQueryInfo->exprList, i);
    if (pExpr->functionId == TSDB_FUNC_TAGPRJ ||
  ...
  if (tagTsColExists) {  // check if the selectivity function exists
    ...
    if ((pQueryInfo->type & TSDB_QUERY_TYPE_PROJECTION_QUERY) != 0) {
      if (numOfAggregation > 0 && pQueryInfo->groupbyExpr.columnInfo.empty()) {
        return invalidSqlErrMsg(pCmd->payload, msg3);
      }
  ...
  for (const auto &col : pQueryInfo->groupbyExpr.columnInfo) {
    ...
    if (colIndex == TSDB_TBNAME_COLUMN_INDEX) {
      ...
      if (TSDB_COL_IS_TAG(col.flag)) {
        const SSchema* tagSchema = pTableMetaInfo->pTableMeta->getTagSchema();
    ...
    if (TSDB_COL_IS_TAG(col.flag)) {
      ...
      // NOTE: tag column does not add to source column list
      SColumnList ids = getColumnList(1, 0, col.colIndex);
      insertResultField(pQueryInfo, (int32_t)size, &ids, bytes, (int8_t)type, name, pExpr);
    } else {
       ...
       */
      if (!hasGroupColumn) {
        doAddGroupColumnForSubquery(pQueryInfo, col);
      }
    }
  }
...
int32_t doFunctionsCompatibleCheck(SSqlCmd* pCmd, SQueryInfo* pQueryInfo) {
  ...
  if (tscQueryTags(pQueryInfo)) {
    ...
    if (pQueryInfo->groupbyExpr.columnInfo.size() > 0 || pQueryInfo->interval.interval > 0) {
      return invalidSqlErrMsg(pCmd->payload, msg4);
    } else {
      return TSDB_CODE_SUCCESS;
    }
  ...
  if (pQueryInfo->groupbyExpr.columnInfo.size() > 0) {
    ...
    for (int32_t i = 0; i < size; ++i) {
      ...
      if (functId == TSDB_FUNC_PRJ && pExpr->colInfo.colId != PRIMARYKEY_TIMESTAMP_COL_INDEX) {
        ...
        for (const auto &col : pQueryInfo->groupbyExpr.columnInfo) {
          if (col.colId == pExpr->colInfo.colId) {
            qualified = true;
            break;
          }
        }
...
int32_t doLocalQueryProcess(SSqlCmd* pCmd, SQueryInfo* pQueryInfo, SQuerySQL* pQuerySql) {
  ...
  switch (index) {
    ...
    default: { return invalidSqlErrMsg(pCmd->payload, msg3); }
  }
...
int32_t doCheckForCreateFromStable(SSqlObj* pSql, SSqlInfo* pInfo) {
  ...
  for(int32_t j = 0; j < numOfTables; ++j) {
    SCreatedTableInfo* pCreateTableInfo = (SCreatedTableInfo*)taosArrayGet(pCreateTable->childTableInfo, j);
    ...
    if (pStableMetaInfo->pTableMeta->numOfTags() != size) {
      return invalidSqlErrMsg(pCmd->payload, msg5);
    }
    ...
    int32_t ret = TSDB_CODE_SUCCESS;
    for (int32_t i = 0; i < size; ++i) {
      const SSchema*          pSchema = &pTagSchema[i];
      tVariantListItem* pItem = (tVariantListItem*)taosArrayGet(pList, i);
    ...
    if (pTag->data == NULL) {
      pTag->data = (char*)malloc(pTag->dataLen);
    }
...
int32_t doCheckForStream(SSqlObj* pSql, SSqlInfo* pInfo) {
  ...
  tVariantListItem* p1 = (tVariantListItem*)taosArrayGet(pSrcMeterName, 0);
  SStrToken srcToken = { .n = p1->pVar.nLen, .type = TK_STRING, .z = p1->pVar.pz };
  ...
  if (pQuerySql->fillType != NULL) {
    ...
    tVariantListItem* pItem = (tVariantListItem*)taosArrayGet(pQuerySql->fillType, 0);
    if (pItem->pVar.nType == TSDB_DATA_TYPE_BINARY) {
...
int32_t doCheckForQuery(SSqlObj* pSql, SQuerySQL* pQuerySql, int32_t index) {
  ...
  for (int32_t i = 0; i < fromSize; ) {
    tVariantListItem* item = (tVariantListItem*)taosArrayGet(pQuerySql->from, i);
    ...
    SStrToken tableName = {.n = pTableItem->nLen, .type = TK_STRING, .z = pTableItem->pz };
    ...
    SStrToken t = {.n = pTableItem->nLen, .type = TSDB_DATA_TYPE_BINARY, .z = pTableItem->pz};
    code = tscSetTableFullName(pTableMetaInfo1, &t, pSql);
    if (code != TSDB_CODE_SUCCESS) {
      return code;
    }

    tVariantListItem* p1 = (tVariantListItem*)taosArrayGet(pQuerySql->from, i + 1);
    if (p1->pVar.nType != TSDB_DATA_TYPE_BINARY) {
      return invalidSqlErrMsg(pCmd->payload, msg7);
    }

    SStrToken aliasName = { .n = p1->pVar.nLen, .type = TK_STRING, .z = p1->pVar.pz };
  ...
  if (pQuerySql->pWhere != NULL) {
    ...
    if (taosArrayGetSize(pQuerySql->from) > 2) { // it is a join query, no wher clause is not allowed.
      return invalidSqlErrMsg(pCmd->payload, "condition missing for join query ");
    }
  }
  ...
  // set order by info
  if (parseOrderbyClause(pCmd, pQueryInfo, pQuerySql, pTableMetaInfo->pTableMeta->getSchema()) != TSDB_CODE_SUCCESS) {
    return TSDB_CODE_TSC_INVALID_SQL;
  }
  ...
  if (pSqlExpr->pLeft == NULL) {
    if (pSqlExpr->nSQLOptr >= TK_BOOL && pSqlExpr->nSQLOptr <= TK_STRING) {
      *pExpr = (tExprNode*)calloc(1, sizeof(tExprNode));
      ...
      (*pExpr)->pVal = (tVariant*)calloc(1, sizeof(tVariant));
      ...
      *pExpr = (tExprNode*)calloc(1, sizeof(tExprNode));
      ...
      (*pExpr)->pSchema = (SSchema*)calloc(1, sizeof(SSchema));
      ...
      for (int32_t i = 0; i < size; ++i) {
        SSqlExpr* p1 = (SSqlExpr*)taosArrayGetP(pQueryInfo->exprList, i);
      ...
      *pExpr = (tExprNode*)calloc(1, sizeof(tExprNode));
      ...
      const SSchema* pSchema = pTableMeta->getColumnSchema(index.columnIndex);
      *(*pExpr)->pSchema = *pSchema;
      ...
      return invalidSqlErrMsg(pCmd->payload, "not support filter expression");
    }
