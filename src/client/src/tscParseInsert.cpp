...
int32_t tsParseOneColumnData(const SSchema *pSchema, SStrToken *pToken, char *payload, std::string &msg, char **str, bool primaryKey,
                             int16_t timePrec) {
  int64_t iv;
  int32_t ret;
  char   *endptr = NULL;
  ...
  switch (pSchema->type) {
    ...
    case TSDB_DATA_TYPE_NCHAR:
      if (pToken->type == TK_NULL) {
        ...
        int32_t output = 0;
        if (!taosMbsToUcs4(pToken->z, pToken->n, (char*)varDataVal(payload), pSchema->bytes - VARSTR_HEADER_SIZE, &output)) {
          char buf[512] = {0};
...
int tsParseOneRowData(char **str, STableDataBlocks *pDataBlocks, const SSchema schema[], SParsedDataColInfo *spd, SSqlCmd* pCmd,
  ...
  for (int i = 0; i < spd->numOfAssignedCols; ++i) {
    ...
    const SSchema *pSchema = schema + colIndex;
...
static void tscSetAssignedColumnInfo(SParsedDataColInfo *spd, const SSchema *pSchema, int32_t numOfCols) {
...
int32_t tscAllocateMemIfNeed(STableDataBlocks *pDataBlock, int32_t rowSize, int32_t * numOfRows) {
  ...
  if (remain < rowSize * factor) {
    ...
    char *tmp = (char*)realloc(pDataBlock->pData, (size_t)pDataBlock->nAllocSize);
    if (tmp != NULL) {
...
static int32_t tscCheckIfCreateTable(char **sqlstr, SSqlObj *pSql) {
  ...
  if (sToken.type == TK_USING) {  // create table if not exists according to the super table
    ...
    const SSchema *pTagSchema = pSTableMeterMetaInfo->pTableMeta->getTagSchema();
    const auto& tinfo = pSTableMeterMetaInfo->pTableMeta->getInfo();
    ...
    uint8_t numOfTags = pSTableMeterMetaInfo->pTableMeta->numOfTags();
    ...
    for (int i = 0; i < spd.numOfAssignedCols; ++i) {
      const SSchema* pSchema = pTagSchema + spd.elems[i].colIndex;
    ...
    char* pTag = (char*)realloc(pCmd->tagData.data, pCmd->tagData.dataLen);
    if (pTag == NULL) {
      return TSDB_CODE_TSC_OUT_OF_MEMORY;
    }
...
static int doPackSendDataBlock(SSqlObj *pSql, int32_t numOfRows, STableDataBlocks *pTableDataBlocks) {
  ...
  STableDataBlocks *pDataBlock = (STableDataBlocks*)taosArrayGetP(pCmd->pDataBlocks, 0);
...
static void parseFileSendDataBlock(void *param, TAOS_RES *tres, int code) {
  ...
  if (pCmd->pTableBlockHashList == NULL) {
    pCmd->pTableBlockHashList = taosHashInit(16, taosGetDefaultHashFunction(TSDB_DATA_TYPE_BIGINT), true, HASH_NO_LOCK);
  }
