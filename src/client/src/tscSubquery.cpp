...
SJoinSupporter* tscCreateJoinSupporter(SSqlObj* pSql, int32_t index) {
  auto pSupporter = new SJoinSupporter{};
  if (pSupporter == NULL) {
    return NULL;
  }
...
static int32_t tscLaunchRealSubqueries(SSqlObj* pSql) {
  ...
  for (int32_t i = 0; i < pSql->subState.numOfSub; ++i) {
    ...
    pSupporter = (SJoinSupporter*)pPrevSub->param;
    ...
    if (UTIL_TABLE_IS_SUPER_TABLE(pTableMetaInfo)) {
      ...
      pExpr->param[0] = (tVariant) { .nType = TSDB_DATA_TYPE_BIGINT, .nLen = sizeof(int64_t), .i64 = colId };
      pExpr->numOfParams = 1;
    }
...
static void tidTagRetrieveCallback(void* param, TAOS_RES* tres, int32_t numOfRows) {
  ...
  if (numOfRows > 0) {
    ...
    char* tmp = (char*)realloc(pSupporter->pIdTagList, length);
    if (tmp == NULL) {
...
void tscSetupOutputColumnIndex(SSqlObj* pSql) {
  ...
  pRes->pColumnIndex = (SColumnIndex*)calloc(1, sizeof(SColumnIndex) * numOfExprs);
  if (pRes->pColumnIndex == NULL) {
...
int32_t tscCreateJoinSubquery(SSqlObj *pSql, int16_t tableIndex, SJoinSupporter *pSupporter) {
  ...
  if (QUERY_IS_JOIN_QUERY(pQueryInfo->type)) {
    ...
    if (UTIL_TABLE_IS_SUPER_TABLE(pTableMetaInfo)) { // return the tableId & tag
      ...
      SSchema s1 = {.type = (uint8_t)type, .colId = s->colId, .bytes = bytes};
...
static void doCleanupSubqueries(SSqlObj *pSql, int32_t numOfSubs) {
  ...
  for(int32_t i = 0; i < numOfSubs; ++i) {
    ...
    SRetrieveSupport* pSupport = (SRetrieveSupport*)pSub->param;
...
static void tscFreeRetrieveSup(SSqlObj *pSql) {
  SRetrieveSupport *trsupport = (SRetrieveSupport*)pSql->param;
...
// set no disk space error info
  ...
  tscHandleSubqueryError(trsupport, (SSqlObj*)tres, pParentSql->res.code);
}
...
static void tscRetrieveFromDnodeCallBack(void *param, TAOS_RES *tres, int numOfRows) {
  ...
  if (pParentSql->res.code != TSDB_CODE_SUCCESS) {
    ...
    tscHandleSubqueryError((SRetrieveSupport*)param, (SSqlObj*)tres, numOfRows);
    return;
  }
...
void tscRetrieveDataRes(void *param, TAOS_RES *tres, int code) {
  ...
  if (pParentSql->res.code != TSDB_CODE_SUCCESS) {
    ...
    tscHandleSubqueryError((SRetrieveSupport*)param, (SSqlObj*)tres, code);
    return;
  }
  ...
  if (taos_errno(pSql) != TSDB_CODE_SUCCESS) {
    ...
    tscHandleSubqueryError((SRetrieveSupport*)param, (SSqlObj*)tres, pParentSql->res.code);
    return;
  }
...
int32_t tscHandleInsertRetry(SSqlObj* pParent, SSqlObj* pSql) {
  ...
  STableDataBlocks* pTableDataBlock = (STableDataBlocks*)taosArrayGetP(pParent->cmd.pDataBlocks, pSupporter->index);
...
int32_t tscHandleMultivnodeInsert(SSqlObj *pSql) {
  ...
  while(numOfSub < pSql->subState.numOfSub) {
    auto pSupporter = new SInsertSupporter{};
    if (pSupporter == NULL) {
      goto _error;
    }
    ...
    STableDataBlocks* pTableDataBlock = (STableDataBlocks*)taosArrayGetP(pCmd->pDataBlocks, numOfSub);
...
static void doBuildResFromSubqueries(SSqlObj* pSql) {
  ...
  char* tmp = (char*)realloc(pRes->pRsp, numOfRes * rowSize + sizeof(tFilePage));
  if (tmp == NULL) {
    pRes->code = TSDB_CODE_TSC_OUT_OF_MEMORY;
    return;
  } else {
...
static UNUSED_FUNC void transferNcharData(SSqlObj *pSql, int32_t columnIndex, TAOS_FIELD *pField) {
  ...
  if (pRes->tsrow[columnIndex] != NULL && pField->type == TSDB_DATA_TYPE_NCHAR) {
    ...
    if (pRes->buffer[columnIndex] == NULL) {
      pRes->buffer[columnIndex] = (char*)malloc(pField->bytes + TSDB_NCHAR_SIZE);
    }
...
char *getArithmeticInputSrc(void *param, const char *name, int32_t colId) {
  ...
  for (int32_t i = 0; i < pSupport->numOfCols; ++i) {
    pExpr = (SSqlExpr*)taosArrayGetP((SArray*)pSupport->exprList, i);
...
TAOS_ROW doSetResultRowData(SSqlObj *pSql) {
  ...
  for (int i = 0; i < size; ++i) {
    ...
    if (type != TSDB_DATA_TYPE_BINARY && type != TSDB_DATA_TYPE_NCHAR) {
      pRes->tsrow[i] = isNull((char*)pRes->urow[i], type) ? NULL : pRes->urow[i];
    } else {
      pRes->tsrow[i] = isNull((char*)pRes->urow[i], type) ? NULL : varDataVal(pRes->urow[i]);
