...
static void tscInitSqlContext(SSqlCmd *pCmd, SLocalReducer *pReducer, tOrderDescriptor *pDesc) {
  ...
  for (int32_t i = 0; i < size; ++i) {
    ...
    pCtx->resultInfo = (SResultRowCellInfo*)calloc(1, pCtx->interBufBytes + sizeof(SResultRowCellInfo));
...
void tscCreateLocalReducer(tExtMemBuffer **pMemBuffer, int32_t numOfBuffer, tOrderDescriptor *pDesc,
  ...
  SCompareParam *param = (SCompareParam *)malloc(sizeof(SCompareParam));
  if (param == NULL) {
    tfree(pReducer);
    return;
  }
  ...
  pReducer->pFinalRes = (char *)calloc(1, pReducer->rowSize * pReducer->resColModel->capacity);
...
static void doExecuteSecondaryMerge(SSqlCmd *pCmd, SLocalReducer *pLocalReducer, bool needInit) {
  ...
  for (int32_t j = 0; j < size; ++j) {
    ...
    if (functionId == TSDB_FUNC_TAG_DUMMY || functionId == TSDB_FUNC_TAG || functionId == TSDB_FUNC_TS_DUMMY) {
      tVariantDestroy(&pCtx->tag);
      char* input = (char*)pCtx->aInputElemBuf;
      
      if (pCtx->inputType == TSDB_DATA_TYPE_BINARY || pCtx->inputType == TSDB_DATA_TYPE_NCHAR) {
        assert(varDataLen(input) <= pCtx->inputBytes);
        tVariantCreateFromBinary(&pCtx->tag, (char*)varDataVal(input), varDataLen(input), pCtx->inputType);
      } else {
...
static void fillMultiRowsOfTagsVal(SQueryInfo *pQueryInfo, int32_t numOfRes, SLocalReducer *pLocalReducer) {
  ...
  char *buf = (char*)malloc((size_t)maxBufSize);
  for (int32_t k = 0; k < size; ++k) {
...
int32_t tscDoLocalMerge(SSqlObj *pSql) {
  ...
  if (pRes == NULL || pRes->pLocalReducer == NULL) {  // all data has been processed
...
int32_t doArithmeticCalculate(SQueryInfo* pQueryInfo, tFilePage* pOutput, int32_t rowSize, int32_t finalRowSize) {
  ...
  char* pbuf = (char*)calloc(1, (size_t)(pOutput->num * maxRowSize));
